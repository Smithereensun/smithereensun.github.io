{
  ...
  "has_date": true,
  "description": "本文以 Linux 2.6.32 版本的内核作为参考 什么是 TCP 半连接队列和全连接队列？ 在 TCP 三次握手的时候，Linux 内核会维护两个队列，分别是： 半连接队列，也称 SYN 队列； 全连接队列，也称 accept 队列； 服务端收到客户端发起的 SYN 请求后，**内核会把该连接存",
  "tags": [
    "计算机基础"
  ],
  "source": "local-markdown-library",
  "source_path": "cs-basics/network/02-tcp3-queue - TCP - 半连接队列和全连接队列.md",
  "date": "2025-05-17"
...
服务端收到客户端发起的 SYN 请求后，**内核会把该连接存储到半连接队列**，并向客户端响应 SYN+ACK，接着客户端会返回 ACK，服务端收到第三次握手的 ACK 后，这也是TCP的三次握手过程，第三次握手之后**内核会把连接从半连接队列移除，然后创建新的完全的连接，并将其添加到 accept 队列，等待进程调用 accept 函数时把连接取出来。**
![](./images/images/7b4345064666-202406222012501.png)
不管是半连接队列还是全连接队列，都有最大长度限制，超过限制时，内核会直接丢弃，或返回 RST 包。

## [TCP 全连接队列溢出](#tcp-全连接队列溢出)

如何知道应用程序的 TCP 全连接队列大小？在服务端可以使用 `ss` 命令，来查看 TCP 全连接队列的情况：

但需要注意的是 `ss` 命令获取的 `Recv-Q/Send-Q` 在「LISTEN 状态」和「非 LISTEN 状态」所表达的含义是不同的。从下面的内核代码可以看出区别：
![](./images/images/45804ec22e6e-202406222012513.png)
在「LISTEN 状态」时，`Recv-Q/Send-Q` 表示的含义如下：
![](./images/images/22ffef35060c-202406222012442.png)
...
![](./images/images/cf4a40a41df8-202406222012495.png)

- Recv-Q：已收到但未被应用进程读取的字节数；

- Send-Q：已发送但未收到确认的字节数；

### [模拟 TCP 全连接队列溢出场景](#模拟-tcp-全连接队列溢出场景)
![测试环境](./images/images/ffb12f63f42c-202406222012476.png)测试环境
实验环境：
...
![](./images/images/e68dcd7dd2a0-202406222012487.png)
在服务端可以使用 `ss` 命令，来查看当前 TCP 全连接队列的情况：
![](./images/images/3509297e4af9-202406222012161.png)
其间共执行了两次 ss 命令，从上面的输出结果，可以发现当前 TCP 全连接队列上升到了 129 大小，超过了最大 TCP 全连接队列。

**当超过了 TCP 最大全连接队列，服务端则会丢掉后续进来的 TCP 连接**，丢掉的 TCP 连接的个数会被统计起来，可以使用 netstat -s 命令来查看：
![](./images/images/b9cb2cb0a3ae-202406222012206.png)
...
![](./images/images/90c262e6e3f3-202406222012079.png)
### [参数调整](#参数调整)

Linux 有个参数可以指定当 TCP 全连接队列满了会使用什么策略来回应客户端。

实际上，丢弃连接只是 Linux 的默认行为，还可以选择向客户端发送 RST 复位报文，告诉客户端连接已经建立失败。
![](./images/images/1b9f3e5f2159-202406222012168.png)
...
![](./images/images/87ed72fde789-202406222012408.png)
...
![](./images/images/4ec868618719-202406222012337.png)
现在重新压测，把 TCP 全连接队列**设置大点**，把 `somaxconn` 设置成 5000：
![](./images/images/9a8c4b0d1548-202406222012686.png)
接着把 Nginx 的 backlog 也同样设置成 5000：
![](./images/images/88299761299c-202406222012769.png)
最后要重启 Nginx 服务，因为只有重新调用 `listen()` 函数 TCP 全连接队列才会重新初始化。

重启完后 Nginx 服务后，服务端执行 ss 命令，查看 TCP 全连接队列大小：
![](./images/images/9d18ba39d136-202406222012306.png)
从执行结果，可以发现 TCP 全连接最大值为 5000。

增大 TCP 全连接队列后，继续压测

客户端同样以 3 万个连接并发发送请求给服务端：
![](./images/images/e633e936fcec-202406222012508.png)
服务端执行 `ss` 命令，查看 TCP 全连接队列使用情况：
![](./images/images/1c5facb71c63-202406222012989.png)
从上面的执行结果，可以发现全连接队列使用增长的很快，但是一直都没有超过最大值，所以就不会溢出，那么 `netstat -s` 就不会有 TCP 全连接队列溢出个数的显示：
![](./images/images/efddf81749ba-202406222012998.png)
...
如何查看 TCP 半连接队列长度？很遗憾，TCP 半连接队列长度的长度，没有像全连接队列那样可以用 ss 命令查看。

但是我们可以抓住 TCP 半连接的特点，就是服务端处于 `SYN_RECV` 状态的 TCP 连接，就是 TCP 半连接队列。

于是，我们可以使用如下命令计算当前 TCP 半连接队列长度：
![](./images/images/97a16aa8efef-202406222012303.png)
### [模拟 TCP 半连接队列溢出场景](#模拟-tcp-半连接队列溢出场景)
...
![](./images/images/3c9e624a6f1a-202406222012196.png)
实验环境：
...
![](./images/images/1feea79051d5-202406222012497.png)
当服务端受到 SYN 攻击后，连接服务端 ssh 就会断开了，无法再连上。只能在服务端主机上执行查看当前 TCP 半连接队列大小：
![](./images/images/caa72d4aad81-202406222012565.png)
同时，还可以通过 netstat -s 观察半连接队列溢出的情况：
![](./images/images/6e55f0484e15-202406222012682.png)
...
![](./images/images/adfba92e5ea6-202406222012868.png)
...
![](./images/images/d3386d97b17c-202406222012905.png)
从源码中，我可以得出共有三个条件因队列长度的关系而被丢弃的：
![](./images/images/e4f805a8b416-202406222012041.png)

1. **如果半连接队列满了，并且没有开启 tcp_syncookies，则会丢弃；**

1. **若全连接队列满了，且没有重传 SYN+ACK 包的连接请求多于 1 个，则会丢弃；**

1. **如果没有开启 tcp_syncookies，并且 max_syn_backlog 减去 当前半连接队列长度小于 (max_syn_backlog &gt;&gt; 2)，则会丢弃；**
...
![](./images/images/bf36c4c06107-202406222012186.png)
从上面源码，可以得知：
...
继续跟进代码，看一下是哪里初始化了半连接队列的最大值 max_qlen_log：
![](./images/images/4b729d1d0212-202406222012214.png)
从上面的代码中，可以算出 max_qlen_log 是 8，于是代入到 检测半连接队列是否满的函数 reqsk_queue_is_full ：
![](./images/images/4f72028d13ce-202406222012339.png)
...
![](./images/images/42cd79724796-202406222012417.png)
...
在前面分析 TCP 第一次握手（收到 SYN 包）时会被丢弃的三种条件：
...
服务端环境如下：
![](./images/images/898e6fdc6824-202406222012098.png)
配置完后，服务端要重启 Nginx，因为全连接队列最大值和半连接队列最大值是在 listen() 函数初始化。

根据前面的源码分析，可以计算出半连接队列 max_qlen_log 的最大值为 256：
![](./images/images/51ca252689cc-202406222012580.png)
客户端执行 hping3 发起 SYN 攻击：
![](./images/images/42bbf1e724b2-202406222012888.png)
服务端执行如下命令，查看处于 SYN_RECV 状态的最大个数：
![](./images/images/12e1b00f2954-202406222012927.png)
...
根据前面测试的结果，服务端处于 SYN_RECV 状态的最大个数是 193，正好是触发了条件 3，所以处于 SYN_RECV 状态的个数还没到「理论半连接队列最大值 256」，就已经把 SYN 包丢弃了。
...
在上面是针对 Linux 2.6.32 版本分析的「理论」半连接最大值的算法，可能每个版本有些不同。比如在 Linux 5.0.0 的时候，「理论」半连接最大值就是全连接队列最大值，但依然还是有队列溢出的三个条件：
![](./images/images/f6ddcca72ba4-202406222012055.png)

如果 SYN 半连接队列已满，只能丢弃连接吗？

并不是这样，**开启 syncookies 功能就可以在不使用 SYN 半连接队列的情况下成功建立连接**，在前面源码分析也可以看到这点，当开启了 syncookies 功能就不会丢弃连接。

syncookies 是这么做的：服务器根据当前状态计算出一个值，放在己方发出的 SYN+ACK 报文中发出，当客户端返回 ACK 报文时，取出该值验证，如果合法，就认为连接建立成功，如下图所示。
![](./images/images/bca5e7d2849c-202406222012103.png)
...
![](./images/images/2c0675ffa5fc-202406222012451.png)
...
在前面源码和实验中，得知**要想增大半连接队列，不能只单纯增大 tcp_max_syn_backlog 的值，还需一同增大 somaxconn 和 backlog，也就是增大全连接队列**。否则，只单纯增大 tcp_max_syn_backlog 是无效的。

增大 tcp_max_syn_backlog 和 somaxconn 的方法是修改 Linux 内核参数：
![](./images/images/2f81c4b91fe0-202406222012459.png)
增大 backlog 的方式，每个 Web 服务都不同，比如 Nginx 增大 backlog 的方法如下：
![](./images/images/4f5b2eeaaf32-202406222012641.png)
最后，改变了如上这些参数后，要重启 Nginx 服务，因为半连接队列和全连接队列都是在 listen() 初始化的。

#### [开启 tcp_syncookies 功能](#开启-tcp-syncookies-功能)

开启 tcp_syncookies 功能的方式也很简单，修改 Linux 内核参数：
![](./images/images/c57f4e5615c9-202406222012730.png)
...
![](./images/images/fe5b0819fbdb-202406222012942.png)
