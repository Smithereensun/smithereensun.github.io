{
  ...
  "title": "Kafka - 高级使用",
  "has_date": true,
  "description": "Kafka 生产者 生产者消息发送流程 发送原理 在消息发送的过程中，涉及到两个线程，main线程和sender线程， main线程是消息的生产线程 sender线程是jvm单例的线程，专门用于消息的发送。 在jvm的内存中开辟了一块缓存空间叫RecordAccumulator（消息累加器），用于将",
  "tags": [
    "微服务",
    "消息队列",
    "Kafka"
  ...
  "source_path": "microservices/message-queue/kafka-advancedusage - Kafka - 高级使用.md",
  "date": "2025-12-21"
...
#### [发送原理](#发送原理)
![](./images/images/933923ebc38e-202405182244201.jpg)
...
1. 调用send方法然后经过拦截器经过序列化器，再经过分区器确定消息发送在具体topic下的哪个分区
  ...
  1. 分区器：决定将数据发往哪个分区（一个分区会创建一个队列）

1. 再将数据发送到对应的消息累加器（默认32MB）中，消息累加器是多个双端队列。并且每个队列和主题分区都具有一一映射关系。消息在累加器中，进行合并。达到了**对应的size（batch.size）或者等待超过对应的等待时间([linger.ms](http://linger.ms))**，都会触发sender线程的发送。

1. sender线程有一个请求池，默认缓存五个请求（ max.in.flight.requests.per.connection ），发送消息后，会等待服务端的ack

  1. 如果没收到ack就会重试，默认重试次数是 int 最大值（ retries ）。

  1. 如果ack成功就会删除累加器中的消息批次，并相应到生产端。

当双端队列中的DQueue满足 batch.size 或者 [linger.ms](http://linger.ms) 条件时触发sender线程。
![](./images/images/e73649017746-202405182244256.jpg)
#### [生产者重要参数列表](#生产者重要参数列表)
![](./images/images/d2254294c300-202405191142710.png)
### [发送](#发送)

#### [普通异步发送](#普通异步发送)

1）需求：创建 Kafka 生产者，采用异步的方式发送到 Kafka Broker
![](./images/images/688c5d264976-202405182244227.jpg)
2）代码编写
...
![](./images/images/411293cdac53-202405182244127.gif)
注意：消息发送失败会自动重试，不需要我们在回调函数中手动重试。
![](./images/images/106b5c49b3b3-202405182244165.jpg)
#### [同步发送API](#同步发送api)

1. 先处理已经堆积在DQueue中的数据。

1. RecordAccumulator再处理外部数据。

同步发送原理：
![](./images/images/78a60a05f7cb-202405182244310.jpg)
只需在异步发送的基础上，再调用一下 get()方法即可。
![](./images/images/7a725f33dd6b-202405182244362.jpg)
### [生产者拦截器](#生产者拦截器)
...
![](./images/images/05cf9b64cf17-202405182244278.jpg)
#### [默认的分区器 DefaultPartitioner](#默认的分区器-defaultpartitioner)

在 IDEA 中 ctrl +n，全局查找 DefaultPartitioner。
![](./images/images/b75cb6c32f33-202405182244313.jpg)
Kafka支持三种分区策略
...
![](./images/images/f605545726de-202405182244349.jpg)
#### [自定义分区器](#自定义分区器)
...
### [生产者如何提高吞吐量](#生产者如何提高吞吐量)

通过提高吞吐量达到低延迟的效果
![](./images/images/00d491ebef40-202405182244387.jpg)
Batch.size 与 [linger.ms](http://linger.ms) 配合使用，根据生成数据的大小指定。
...
![](./images/images/c3a52df2577f-202405182244449.jpg)
...
![](./images/images/cad3aa6ec025-202405182244091.jpg)
...
![](./images/images/45a38b46e75a-202405191654317.png)
...
![](./images/images/587e18059899-202405182244101.jpg)
开启事务需要producer设置 [transactional.id](http://transactional.id) 的值并同时开启幂等性。

通过事务协调器，来实现事务，相关API如下：
![](./images/images/7fa46784f7c8-202405182244067.jpg)
### [数据有序](#数据有序)

消息在单分区内有序，多分区内无序（如果对多分区进行排序，造成分区无法工作需要等待排序，浪费性能）
![](./images/images/c9020b308540-202405182326753.png)
...
![](./images/images/002948bc5d21-202405182326674.png)
...
![](./images/images/9b31a9c43fb8-202405182328769.png)
Controller节点的主要职能：

1. **Broker状态管理**：Controller会跟踪集群中所有Broker的在线状态，并在Broker宕机或者恢复时更新集群的状态。

1. **分区状态管理**：当新的Topic被创建，或者已有的Topic被删除时，Controller会负责管理这些变化，并更新集群的状态。

1. **分区领导者选举**：当一台Broker节点宕机时，并且宕机的机器上包含分区领导者副本时，Controller会负责对其上的所有Partition进行新的领导者选举。

1. **副本状态管理**：Controller负责管理Partition的ISR列表，当Follower副本无法及时跟随Leader副本时，Controller会将其从ISR列表中移除。

1. **分区重平衡**：当添加或删除Broker节点时，Controller会负责对Partition的分布进行重平衡，以确保数据的均匀分布。

1. **存储集群元数据**：Controller保存了集群中最全的元数据信息，并通过发送请求同步到其他Broker上面。

### [Zookeeper作用](#zookeeper作用)

Zookeeper在Kafka中扮演了重要的角色，kafka使用zookeeper进行元数据管理，保存broker注册信息，包括主题（Topic）、分区（Partition）信息等，选择分区leader。
![](./images/images/238c34c77ec4-202405191029053.png)
### [Broker选举Leader](#broker选举leader)
...
![](./images/images/0ec542bcf3b0-202405191030894.png)
详细图：
...
![](./images/images/d5cb40e6b95b-202405191030349.png)
流程如下：
...
#### [Broker重要参数](#broker重要参数)
![](./images/images/53caac82ac43-202405191033922.png)![](./images/images/2b9c05d988c6-202405191033143.png)
### [节点服役和退役](#节点服役和退役)
...
![](./images/images/23a3c94b59af-202405191037772.png)
...
#### [副本选举 Leader](#副本选举-leader)

Kafka 集群中有一个 broker 的 Controller 会被选举为 Controller Leader，负责管理集群Broker 的上下线，所有 topic 的分区副本分配和 Leader 选举等工作。

##### [触发副本leader的选举的时机](#触发副本leader的选举的时机)
...
- **手动触发：**通过 Kafka 管理工具（[kafka-preferred-replica-election.sh](http://kafka-preferred-replica-election.sh)），可以手动触发选举，以平衡负载或实现集群维护。

##### [Leader Replica 选举策略](#leader-replica-选举策略)
...
1.
首选副本选举策略（Preferred Replica Election）：首选副本选举策略也是 Kafka 默认的选举策略。在这种策略下，每个分区都有一个首选副本（Preferred Replica），通常是副本集合中的第一个副本。当触发选举时，控制器会优先选择该首选副本作为新的 Leader Replica，只有在首选副本不可用的情况下，才会考虑其他副本。当然，也可以使用命令手动指定每个分区的首选副本：

1. 不干净副本选举策略（Unclean Leader Election）：在某些情况下，ISR 选举策略可能会失败，例如当所有 ISR 副本都不可用时。在这种情况下，可以使用 Unclean Leader 选举策略。Unclean Leader 选举策略会从所有副本中（包含OSR集合）选择一个副本作为新的 Leader 副本，即使这个副本与当前 Leader 副本不同步。这种选举策略可能会导致数据丢失，因此只应在紧急情况下使用。
 修改下面的配置，可以开启 Unclean Leader 选举策略，默认关闭。

Broker中Controller 的信息同步工作是依赖于 Zookeeper 的 ./broker/topic 目录下的信息。
![](./images/images/9ca95c3870d1-202405191038640.png)
...
如果follower落后leader过多，体现在落后时间 [repca.lag.time.max.ms](http://repca.lag.time.max.ms)，或者落后偏移量repca.lag.max.messages(由于kafka生成速度不好界定，后面取消了该参数)，follower就会被移除ISR队列，等待该队列LEO追上HW，才会重新加入ISR中。
![](./images/images/5e1c37d7921b-202405191040094.png)
...
![](./images/images/ae57692ae7b5-202405191040346.png)
#### [分区副本分配](#分区副本分配)
...
（2）查看分区和副本情况。
![](./images/images/c3292457224e-202405191041957.png)
#### [手动调整分区副本](#手动调整分区副本)
![](./images/images/06daef65f340-202405191042767.png)
手动调整分区副本存储的步骤如下：
...
![](./images/images/96a5f8666847-202405191043396.png)
...
![](./images/images/f2954abdf445-202405191046998.png)
...
![](./images/images/d1a92fa07b17-202405191047057.png)
...
![](./images/images/aa135f4cbd5a-202405191047667.png)
#### [文件清理策略](#文件清理策略)
...
- [log.retention.ms](http://log.retention.ms)：最高优先级毫秒

- [log.retention.check.interval.ms](http://log.retention.check.interval.ms)：负责设置检查周期，默认5分钟

- [file.delete.delay.ms](http://file.delete.delay.ms)：延迟执行删除时间
...
- 日志删除任务会周期检查当前日志文件中是否有保留时间超过设定的阈值来寻找可删除的日志段文件集合；这里需要注意log.retention参数的优先级：[log.retention.ms](http://log.retention.ms) &gt; log.retention.minutes &gt; log.retention.hours，默认只会配置log.retention.hours参数，值为168即为7天。

- 删除过期的日志段文件，并不是简单的根据日志段文件的修改时间计算，而是要根据该日志段中最大的时间戳来计算的，首先要查询该日志分段所对应的时间戳索引文件，查找该时间戳索引文件的最后一条索引数据，如果时间戳大于0就取值，否则才会使用最近修改时间。

- 在删除的时候先从Log对象所维护的日志段的跳跃表中移除要删除的日志段，用来确保已经没有线程来读取这些日志段；接着将日志段所对应的所有文件，包括索引文件都添加上**.deleted的后缀；最后交给一个以delete-file命名的延迟任务来删除这些以.deleted为后缀的文件，默认是1分钟执行一次，[可以通过file.delete.delay.ms](http://xn--file-955fz0ybn7i7sa.delete.delay.ms)**来配置。
...
- 基于日志起始偏移量
  ...
  - 该策略判断依据是日志段的下一个日志段的起始偏移量 baseOffset是否小于等于 logStartOffset，如果是，则可以删除此日志分段。这里说一下logStartOffset，一般情况下，日志文件的起始偏移量 logStartOffset等于第一个日志分段的 baseOffset，但这并不是绝对的，logStartOffset的值可以通过 DeleteRecordsRequest请求、使用 [kafka-delete-records.sh](http://kafka-delete-records.sh) 脚本、日志的清理和截断等操作进行修改。
...
![](./images/images/5734a3e727a6-202405191049264.png)
#### [页缓存](#页缓存)
...
零拷贝 并不是不需要拷贝，而是减少不必要的拷贝次数，通常使用在IO读写过程中。常规应用程序IO过程如下图，会经过四次拷贝：
...
![](./images/images/401379ca4dca-202405191051139.png)
...
![](./images/images/cdecde87a9a5-202405191057142.png)
需要注意的是：
...
![](./images/images/f10d8b6513ac-202405191057925.png)
...
#### [消费者组选举Leader](#消费者组选举leader)

具体的消费者组初始化流程：

通过对GroupId进行Hash得到获得服务器的coordinator，coordinator负责选出消费组中的Leader，并且协调信息。真正存储消费记录的是 _consumer_offsets_partition 。
![](./images/images/06ee5c98d74b-202405191058931.png)![](./images/images/8f5b29619cc5-202405191058765.png)![](./images/images/cc9a87526ac7-202405191058773.png)![](./images/images/d2bd8378c49c-202405191059964.png)
### [消费者API](#消费者api)

消费组单消费者以及消费者组多消费者
![](./images/images/86ed1da16b0b-202405191059304.png)
注意：在消费者 API 代码中**必须配置消费者组 id**。命令行启动消费者不填写消费者组id 会被自动填写随机的消费者组 id。

### [分区平衡以及再平衡](#分区平衡以及再平衡)
![](./images/images/8e90ec2ee450-202405191100921.png)

- [heartbeat.interval.ms](http://heartbeat.interval.ms) Kafka 消费者和 coordinator 之间的心跳时间，默认 3s。该条目的值必须小于 [session.timeout.ms](http://session.timeout.ms)，[也不应该高于session.timeout.ms](http://xn--session-3w3kyxxoq96lrw3hqvsb.timeout.ms) 的 1/3。

- [session.timeout.ms](http://session.timeout.ms) Kafka 消费者和 coordinator 之间连接超时时间，默认 45s。超过该值，该消费者被移除，消费者组执行再平衡。

- [max.poll.interval.ms](http://max.poll.interval.ms) 消费者处理消息的最大时长，默认是 5 分钟。超过该值，该消费者被移除，消费者组执行再平衡。
...
#### [Range](#range)
![](./images/images/3b41d7e4c437-202405191100580.png)
Range 分区分配再平衡案例：
...
![](./images/images/f845548422d3-202405191106159.png)
...
1. 当 Rebalance(重分配，后面会详细分析) 发生时，尽量与上一次分配结果保持一致。

该算法的精髓在于，重分配后，还能尽量与上一次结果保持一致，进而达到消费者故障下线，故障恢复后的均衡问题，在此就不举例了。

### [offset位移提交](#offset位移提交)

#### [offset 的默认维护位置](#offset-的默认维护位置)
...
![](./images/images/a970687ae2b3-202405191107383.png)

- enable.auto.commit：是否开启自动提交offset功能，默认是true

- [auto.commit.interval.ms](http://auto.commit.interval.ms)：自动提交offset的时间间隔，默认是5s
...
![](./images/images/2b98bfa1db40-202405191108631.png)
#### [指定消费位置](#指定消费位置)
...
![](./images/images/207a79e52a46-202405191108717.png)
...
![](./images/images/7632795be207-202405191110531.png)
#### [消费者事务](#消费者事务)
![](./images/images/52d2cad0b57e-202405191111570.png)
### [数据积压（提高吞吐量）](#数据积压-提高吞吐量)
![](./images/images/7f14f60ef6d7-202405191111584.png)
