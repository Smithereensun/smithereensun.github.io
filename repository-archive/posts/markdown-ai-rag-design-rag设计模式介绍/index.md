{
  ...
  "title": "RAG设计模式介绍",
  "has_date": true,
  "description": "前面，在讲述AI AGENT设计模式时，从最经典的ReAct模式开始，沿着规划路线介绍了REWOO、Plan&Execute和LLM Compiler，沿着反思路线介绍了Basic Reflection、Self Discover和Reflexion，并以最强大的设计模式LATS作为收尾。 但是，所",
  "tags": [
    "AI",
    "RAG"
  ],
  "source": "local-markdown-library",
  "source_path": "ai/rag-design - RAG设计模式介绍.md",
  "date": "2026-04-19"
...
解决这个问题的最佳方式是利用RAG技术，接下来我们正式看《RAG实战篇》系列。对于RAG还不太熟悉的朋友，可以先看文章Rag入门知识

## [构建一个最小可行性的Rag系统](#构建一个最小可行性的rag系统)

### [RAG系统实现方案概览](#rag系统实现方案概览)
...
#### [Query Construction（查询构建）](#query-construction-查询构建)
...
### [构建最小化的Naive Rag系统](#构建最小化的naive-rag系统)
...
#### [第一步建立索引](#第一步建立索引)
![](./images/images/b1e9c31f955c-202506081128406.jpg)
...
#### [第三步 生成](#第三步-生成)
![](./images/images/7caccb22f240-202506081230651.jpg)
...
### [Chunk optimization（块优化）](#chunk-optimization-块优化)
...
### [多层表达索引](#多层表达索引)
![](./images/images/64f8eb32e312-202506081234106.jpg)
...
![](./images/images/22f0ca671064-202506081236808.png)
...
![](./images/images/527e80dbd647-202506081240626.jpg)
以下代码中的reciprocal_rank_fusion，就是rag-fusion多出来的一步。
![](./images/images/e3971934a63b-202506081240532.png)
### [Decomposition（问题分解）](#decomposition-问题分解)
...
![](./images/images/7a0b1dd9c761-202506081240309.png)
在最终回答子问题的时候有两种方式。

第一种是递归回答，即先接收一个子问题，先回答这个子问题并接受这个答案，并用它来帮助回答第二个子问题。
![](./images/images/3643c1f35092-202506081241135.jpg)
给出prompt：
![](./images/images/5764c49651e7-202506081241076.png)
下面是递归回答的主逻辑，生成最终回答：
![](./images/images/8231b06aa82f-202506081241990.png)
第二种方式是独立回答，然后再把所有的这些答案串联起来，得出最终答案。这更适合于一组有几个独立的问题，问题之间的答案不互相依赖的情况。
![](./images/images/713ba77ff7ad-202506081241610.jpg)![](./images/images/ee50f1d04824-202506081242388.png)
### [Step-back（Query后退）](#step-back-query后退)
...
![](./images/images/b5d1a6156e24-202506081242289.jpg)
先给大模型提供一些step-back的示例：
![](./images/images/2933625dc44f-202506081243056.png)
然后对输入问题进行step-back
![](./images/images/be4dd06acca7-202506081243562.png)
结合prompt，生成最终回答
![](./images/images/27d17e604b10-202506081243834.png)
### [HYDE](#hyde)
...
![](./images/images/bdc956a8443b-202506081243118.png)
## [优化路由环节](#优化路由环节)
...
### [Logical routing（基于逻辑的路由）](#logical-routing-基于逻辑的路由)

基于逻辑的路由，其原理非常简单。大模型接收到问题之后，根据决策步骤，去选择正确的索引数据库，比如图数据库、向量数据库等等，如下图所示。
![](./images/images/f66fb5797e36-202506081245425.jpg)
其使用函数调用（function calling）来产生结构化输出。
![](./images/images/e215b088dfcc-202506081245526.jpg)
...
### [Sematicrouting（基于语义的路由）](#sematicrouting-基于语义的路由)

基于语义的路由，其原理也非常简单，大模型根据query的语义相似度，去自动配置不同的prompt。
![](./images/images/0e588787e162-202506081247432.jpg)
