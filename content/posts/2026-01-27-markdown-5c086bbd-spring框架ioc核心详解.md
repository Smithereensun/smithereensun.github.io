{
  ...
  "has_date": true,
  "description": "介绍 IoC（Inversion of Control:控制反转） 是一种设计思想，而不是一个具体的技术实现。IoC 的思想就是将原本在程序中手动创建对象的控制权，交由 Spring 框架来管理，由Spring容器管理bean的整个生命周期。通俗来说就是**IoC是设计思想，DI是实现方式。** 通",
  "tags": [
    "框架",
    "Spring"
  ],
  "source": "local-markdown-library",
  "source_path": "framework/spring/ioc1-summary - Spring框架IoC核心详解.md",
  "date": "2026-01-27"
...
源码中也可以看到，用逗号或分号分隔
![](/imported/markdown/2026-01-27-markdown-5c086bbd-spring框架ioc核心详解/images/306dc881fbaa-202601251734461.png)
#### [Java 配置](#java-配置)
...
1. 创建方法，方法上加上@bean，该方法用于创建实例并返回，该实例创建后会交给spring管理，方法名建议与实例名相同（首字母小写）。注：实例类不需要加任何注解
...
1. 设置ComponentScan的basePackage（ComponentScan未设置制定扫描路径或类，会扫描当前类机器子目录下所有被以上注解声明的类）, 比如

  - 在xml文件里设置`&lt;context:component-scan base-package='com..springframework'&gt;`,

  - 或者在配置类中设置`@ComponentScan("com..springframework")`注解

  - 或者 直接在APP类中 `new AnnotationConfigApplicationContext("com..springframework")`指定扫描的basePackage.

## [依赖注入DI的方式](#依赖注入di的方式)

其原理是将对象的依赖关系由外部容器来管理和注入。这样，对象只需要关注自身的核心功能，而不需要关心如何获取依赖对象。它的目的是解耦组件之间的依赖关系，提高代码的灵活性、可维护性和可测试性。符合**依赖倒转原则**
...
1. @Autowired 默认是**根据类型**（byType ）进行自动装配的

1. 如果有多个类型一样的Bean候选者，需要指定按照名称（byName ）进行装配，则需要配合 @Qualifier。指定名称后，如果Spring IOC容器中没有对应的组件bean抛出NoSuchBeanDefinitionException。也可以将@Autowired中required配置为false，如果配置为false之后，当没有找到相应bean的时候，系统不会抛异常
...
1. @Resource可以作用TYPE、FIELD、METHOD上

1. @Resource是**默认根据属性名称**进行自动装配的，如果有多个类型一样的Bean候选者，则可以通过name进行指定进行注入
...
1. @Inject可以作用CONSTRUCTOR、METHOD、FIELD上

1. @Inject是根据类型进行自动装配的，如果需要按名称进行装配，则需要配合@Named；@Named 的作用类似 @Qualifier！
...
### [如何配置 bean 的作用域呢？](#如何配置-bean-的作用域呢)
...
1. **无状态Bean设计（根本性解决）**：在 Bean 中尽量避免定义可变的成员变量。

1. 在类中定义一个 `ThreadLocal` 成员变量，将需要的可变成员变量保存在 `ThreadLocal` 中（推荐的一种方式）。

## [IOC的结构设计](#ioc的结构设计)
![](/imported/markdown/2026-01-27-markdown-5c086bbd-spring框架ioc核心详解/images/e304ed7d4bea-202404281027710.png)
...
![](/imported/markdown/2026-01-27-markdown-5c086bbd-spring框架ioc核心详解/images/8f55b7145a33-202404281026702.png)
...
![](/imported/markdown/2026-01-27-markdown-5c086bbd-spring框架ioc核心详解/images/01e87344aeea-202404281028156.png)
#### [BeanDefinitionReader](#beandefinitionreader)

Bean 的解析过程非常复杂，功能被分的很细，因为这里需要被扩展的地方很多，必须保证有足够的灵活性，以应对可能的变化。Bean 的解析主要就是对 Spring 配置文件的解析。这个解析过程主要通过下图中的类完成：
![](/imported/markdown/2026-01-27-markdown-5c086bbd-spring框架ioc核心详解/images/5642acb7a2ec-202404281029913.png)
#### [BeanDefinitionHolder](#beandefinitionholder)

BeanDefinitionHolder 是BeanDefination的包装类，用来存储BeanDefinition，name以及aliases等
![](/imported/markdown/2026-01-27-markdown-5c086bbd-spring框架ioc核心详解/images/220ccbd48433-202404281029574.png)
...
![](/imported/markdown/2026-01-27-markdown-5c086bbd-spring框架ioc核心详解/images/35df791769e7-202404281030642.png)
...
![](/imported/markdown/2026-01-27-markdown-5c086bbd-spring框架ioc核心详解/images/444c317b8826-202404281030496.png)
