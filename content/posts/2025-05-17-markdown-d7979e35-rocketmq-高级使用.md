{
  ...
  "title": "RocketMQ - 高级使用",
  "has_date": true,
  "description": "消息存储 分布式队列因为有高可靠性的要求，所以数据要进行持久化存储。 消息生成者发送消息 MQ收到消息，将消息进行持久化，在存储中新增一条记录 返回ACK给生产者 MQ push 消息给对应的消费者，然后等待消费者返回ACK 如果消息消费者在指定时间内成功返回ack，那么MQ认为消息消费成功，在存储",
  "tags": [
    "微服务",
    "消息队列",
    "RocketMQ"
  ...
  "source_path": "microservices/message-queue/rocketMQ-advancedusage - RocketMQ - 高级使用.md",
  "date": "2025-05-17"
...
![](/imported/markdown/2025-05-17-markdown-d7979e35-rocketmq-高级使用/images/79eab433bf41-202404291940927.png)

1. 消息生成者发送消息

1. MQ收到消息，将消息进行持久化，在存储中新增一条记录

1. 返回ACK给生产者

1. MQ push 消息给对应的消费者，然后等待消费者返回ACK

1. 如果消息消费者在指定时间内成功返回ack，那么MQ认为消息消费成功，在存储中删除消息，即执行第6步；如果MQ在指定时间内没有收到ACK，则认为消息消费失败，会尝试重新push消息,重复执行4、5、6步骤
...
![](/imported/markdown/2025-05-17-markdown-d7979e35-rocketmq-高级使用/images/f142995cd6fa-202404291940928.png)
...
![](/imported/markdown/2025-05-17-markdown-d7979e35-rocketmq-高级使用/images/eb1c09d7f5c6-202404291940930.png)
...
![](/imported/markdown/2025-05-17-markdown-d7979e35-rocketmq-高级使用/images/33b2734f8de7-202404291940935.png)通过使用mmap的方式，可以省去向用户态的内存复制，提高速度。这种机制在Java中是通过MappedByteBuffer实现的
...
![](/imported/markdown/2025-05-17-markdown-d7979e35-rocketmq-高级使用/images/b58b5d3f7905-202404291940936.png)
...
![](/imported/markdown/2025-05-17-markdown-d7979e35-rocketmq-高级使用/images/4fa0108761fc-202404291940942.png)
1）同步刷盘
...
## [高可用性机制](#高可用性机制)
![](/imported/markdown/2025-05-17-markdown-d7979e35-rocketmq-高级使用/images/08ad383ba39e-202404291940441.jpg)
...
![](/imported/markdown/2025-05-17-markdown-d7979e35-rocketmq-高级使用/images/50a5eb1bce6a-202404291940462.jpg)
### [消息主从复制](#消息主从复制)
...
4）总结
![](/imported/markdown/2025-05-17-markdown-d7979e35-rocketmq-高级使用/images/2031ac25c2cb-202404291940476.png)
...
### [集群Master选举机制](#集群master选举机制)

#### [基于raft协议的过半写入机制](#基于raft协议的过半写入机制)
![](/imported/markdown/2025-05-17-markdown-d7979e35-rocketmq-高级使用/images/162799e30dae-202404291948183.png)
...
#### [基于raft协议的Leader选举机制](#基于raft协议的leader选举机制)
![](/imported/markdown/2025-05-17-markdown-d7979e35-rocketmq-高级使用/images/e34804397963-202404291948019.png)
...
![](/imported/markdown/2025-05-17-markdown-d7979e35-rocketmq-高级使用/images/c4a24985bfd0-202404291949203.png)
...
### [Producer负载均衡](#producer负载均衡)

Producer端，每个实例在发消息的时候，默认会轮询所有的message queue发送，以达到让消息平均落在不同的queue上。而由于queue可以散落在不同的broker，所以消息就发送到不同的broker下，如下图：
![](/imported/markdown/2025-05-17-markdown-d7979e35-rocketmq-高级使用/images/d5531486f934-202404291940491.png)
图中箭头线条上的标号代表顺序，发布方会把第一条消息发送至 Queue 0，然后第二条消息发送至 Queue 1，以此类推。

### [Consumer负载均衡](#consumer负载均衡)
...
![](/imported/markdown/2025-05-17-markdown-d7979e35-rocketmq-高级使用/images/ef6f2c795e35-202404291940517.png)
还有另外一种平均的算法是AllocateMessageQueueAveragelyByCircle，也是平均分摊每一条queue，只是以环状轮流分queue的形式，如下图：
![](/imported/markdown/2025-05-17-markdown-d7979e35-rocketmq-高级使用/images/b50624a14527-202404291940538.png)
...
![](/imported/markdown/2025-05-17-markdown-d7979e35-rocketmq-高级使用/images/83aed480af9f-202404291940994.png)
## [消息重试](#消息重试)
...
![](/imported/markdown/2025-05-17-markdown-d7979e35-rocketmq-高级使用/images/fc1410feaedc-202404291940011.png)

1. 在消息界面根据主题查询死信消息

![](/imported/markdown/2025-05-17-markdown-d7979e35-rocketmq-高级使用/images/bb6fe0d90b95-202404291940027.png)
