{
  ...
  "has_date": true,
  "description": "背景 Paxos 算法是 Leslie Lamport莱斯利·兰伯特在 **1990** 年提出了一种分布式系统 **共识** 算法。这也是第一个被证明完备的共识算法（前提是不存在拜占庭将军问题，也就是没有恶意节点）。 为了介绍 Paxos 算法，兰伯特专门写了一篇幽默风趣的论文。在这篇论文中，他虚",
  "tags": [
    "微服务"
  ],
  "source": "local-markdown-library",
  "source_path": "microservices/protocol/paxos-detail - Paxos算法：如何解决分布式系统中的共识问题？.md",
  "date": "2025-05-17"
...
![](/imported/markdown/2025-05-17-markdown-4ed893d4-paxos算法-如何解决分布式系统中的共识问题/images/b46d23ba3386-202501191329940.png)
### [角色](#角色)
...
1. 接受者（Acceptor）：也可以叫做投票员（voter），负责对提议者的提案进行投票，同时需要记住自己的投票历史；

1. 学习者（Learner）：如果有超过半数接受者就某个提议达成了共识，那么学习者就需要接受这个提议，并就该提议作出运算，然后将运算结果返回给客户端。

![](/imported/markdown/2025-05-17-markdown-4ed893d4-paxos算法-如何解决分布式系统中的共识问题/images/4591e3c24011-202501152154475.png)
...
![](/imported/markdown/2025-05-17-markdown-4ed893d4-paxos算法-如何解决分布式系统中的共识问题/images/bef0ba4d3703-202501191325433.jpg)
...
![](/imported/markdown/2025-05-17-markdown-4ed893d4-paxos算法-如何解决分布式系统中的共识问题/images/e9936ec1c1b2-202501191331061.png)
接下来，节点 A，B 接收到客户端 1 的准备请求（提案编号为 1），节点 C 接收到客户端 2 的准备请求（提案编号为 5）。
![](/imported/markdown/2025-05-17-markdown-4ed893d4-paxos算法-如何解决分布式系统中的共识问题/images/2cbbd0929167-202501191331000.png)
...
![](/imported/markdown/2025-05-17-markdown-4ed893d4-paxos算法-如何解决分布式系统中的共识问题/images/69f553798ea1-202501191332828.png)
...
1. 如果一个接受者收到一个编号为Mn的准备请求，并且编号Mn大于它已经响应的所有准备请求的编号，那么它就会将它已经批准过的最大编号的提案作为响应反馈给提议者，同时该接受者会承诺不会再批准任何编号小于Mn的提案。
...
![](/imported/markdown/2025-05-17-markdown-4ed893d4-paxos算法-如何解决分布式系统中的共识问题/images/5af06bd1ad08-202501191335392.png)
...
![](/imported/markdown/2025-05-17-markdown-4ed893d4-paxos算法-如何解决分布式系统中的共识问题/images/80a5016f2c1f-202501191336511.png)
...
1. 如果接受者收到这个针对[Mn,Vn]提案的接受请求，只要该接受者尚未对编号大于Mn的准备请求做出响应，它就可以通过这个提案。

当提议者收到了多数接受者的接受应答后，协商结束，共识决议形成，将形成的决议发送给所有学习节点进行学习。

所以Paxos算法的整体详细流程如下：
![](/imported/markdown/2025-05-17-markdown-4ed893d4-paxos算法-如何解决分布式系统中的共识问题/images/a6faf22b9a9c-202501191327179.jpg)
#### [接受者存在已通过提案的情况](#接受者存在已通过提案的情况)
...
![](/imported/markdown/2025-05-17-markdown-4ed893d4-paxos算法-如何解决分布式系统中的共识问题/images/c8e82c8408f4-202501191337073.png)
节点 A, B 接收到客户端 3 的准备请求，由于节点 A, B 已通过提案 [5, "百里"]，故在准备响应中，包含此提案信息。

节点 C 接收到客户端 3 的准备请求，由于节点 C 未通过任何提案，故节点 C 将返回“尚无提案”的准备响应。
![](/imported/markdown/2025-05-17-markdown-4ed893d4-paxos算法-如何解决分布式系统中的共识问题/images/bdd2032b39d2-202501191338483.png)
...
![](/imported/markdown/2025-05-17-markdown-4ed893d4-paxos算法-如何解决分布式系统中的共识问题/images/0fd2ed215969-202501191338094.png)
节点 A, B, C 接收到客户端 3 的接受请求，由于提案编号 8 不小于三个节点承诺可以通过的最小提案编号，故均通过提案 [8, 百里]。
![](/imported/markdown/2025-05-17-markdown-4ed893d4-paxos算法-如何解决分布式系统中的共识问题/images/9189519c7677-202501191338831.png)
...
![](/imported/markdown/2025-05-17-markdown-4ed893d4-paxos算法-如何解决分布式系统中的共识问题/images/cbc912797699-202501191341875.png)
Lamport 没有说明如何选举领导者，需要在实现 Multi Paxos 算法的时候自行实现。这里我们略去如何选举领导者的算法，假设已经选举出领导者。

### [优化 Basic Paxos 执行过程](#优化-basic-paxos-执行过程)
...
### [Chubby 的 Multi Paxos 实现](#chubby-的-multi-paxos-实现)
...
![](/imported/markdown/2025-05-17-markdown-4ed893d4-paxos算法-如何解决分布式系统中的共识问题/images/e87b1c2249ac-202501191342574.png)

1. Chubby 所有的读请求由主节点来处理：当主节点接收到读请求，主节点只需要查询本地数据，然后返回给客户端。

![](/imported/markdown/2025-05-17-markdown-4ed893d4-paxos算法-如何解决分布式系统中的共识问题/images/98d410faf89f-202501191343202.png)
