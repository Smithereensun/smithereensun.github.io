{
  ...
  "title": "Agent常见面试题",
  "has_date": false,
  "description": "什么是大模型 Agent？它与传统的 AI 系统有什么不同？ 大模型 Agent 是基于大型语言模型并结合模块化规划、记忆和工具调用的自主决策系统，它能够根据最终目标把复杂任务拆分成子任务，调用 API、检索数据库或使用插件，再通过内部循环不断优化执行流程，基本不需要人在每一步都监督。 传统 AI ",
  "tags": [
    "面试",
    "AI",
    "Agent"
  ...
  "source_path": "interview/ai/agent - Agent常见面试题.md"

}

---

## [什么是大模型 Agent？它与传统的 AI 系统有什么不同？](#什么是大模型-agent-它与传统的-ai-系统有什么不同)
...
## [什么是 AI Agent？其核心思想是什么？](#什么是-ai-agent-其核心思想是什么)
...
1. 规划：基于输入，LLM生成多步行动计划，拆解为子任务并排序，就像制定详细食谱确保烹饪思路清晰。

1. 决策：基于记忆内容和任务目标，Agent判断是否继续执行下一步操作或结束流程

1. 工具调用：Agent根据计划动态选择并调用外部工具或API(如搜索、数据库、计算器等)来完成各子任务。

1. 观测：在每次工具执行后，Agent将输出作为“Observation”反馈给LLM，用于更新当前状态和后续决策

1. 记忆：关键观测和交互细节被存入短期或长期记忆，提高多轮对话或复杂任务中的上下文一致性。

1. 决策：基于最新的观测结果、记忆内容和任务目标，Agent判断是否继续执行下一步操作或结束流程

1. 输出：在所有必要步骤完成后，Agent汇总信息，生成并返回最终结果给用户。

1. 反思与纠错：可选地启动自我反思模块，评估已执行行动的正确性，必要时重规划或修正策略，避免重复错误。
...
## [什么是 Agent Loop？其工作流程是什么？](#什么是-agent-loop-其工作流程是什么)

Agent Loop 是所有 Agent 范式共享的运行引擎，其本质是一个 `while` 循环：每一次迭代完成"LLM 推理 → 工具调用 → 上下文更新"的完整链路，直至任务终止。
![](/imported/markdown/undated-markdown-12063518-agent常见面试题/images/67eaa2c05df7-202603102211214.png)
**标准工作流：**

1. **初始化**：加载 System Prompt、可用工具列表及用户初始请求，组装第一轮上下文。

1. **循环迭代**（核心）：读取当前完整上下文 → LLM 推理决定下一步行动（调用工具 or 直接回复）→ 触发并执行对应工具 → 捕获工具返回结果（Observation）→ 将 Observation 追加至上下文。

1. **终止条件**：当 LLM 在某轮判断任务完成，直接输出最终回复而不再调用工具时，退出循环。

1. **安全兜底**：为防止模型陷入死循环，须设置强制中断条件，如最大迭代轮次上限（通常 10 ～ 20 轮）或 Token 消耗阈值。
...
![](/imported/markdown/undated-markdown-12063518-agent常见面试题/images/721e52caba4b-202603180859126.png)
...
## [什么是 CoT 思维链？如何实现 CoT 思维链？](#什么是-cot-思维链-如何实现-cot-思维链)
...
## [什么是 CoT 思维链和 ReAct 模式？它们如何提高 AI 推理能力？](#什么是-cot-思维链和-react-模式-它们如何提高-ai-推理能力)
...
## [ReAct 是什么？说说它的原理](#react-是什么-说说它的原理)
...
1. 行动(Action)：根据当前思路选择合适的操作 (如调用搜索、工具 API、环境交互等)，并输出具体的指令。

1. 观察(Observation)：执行Action 后，系统将返回的结果(网页片段、环境反馈等)提供给模型。

1. 循环迭代：将 Observation 附加到上下文，模型在新的上下文中继续“思考一行动一观察”循环，直到输出最终答案或结束指令
...
## [什么是 ReAct？如何基于 ReAct 模式构建具备自主规划能力的 AI 智能体？](#什么是-react-如何基于-react-模式构建具备自主规划能力的-ai-智能体)
...
![](/imported/markdown/undated-markdown-12063518-agent常见面试题/images/0533130bfa7b-202508031634674.png)
...
1. **行动 (Action)：** 调用监控查询工具 `query_monitor(service="user-service", time="morning")`

1. **观察 (Observation)：** 返回数据表明，9:00-9:30 期间 CPU 利用率飙升至 98%，同时伴随大量慢 SQL 告警。

1. **思考 (Thought)：** 既然有慢 SQL 告警，说明数据库查询可能存在瓶颈。我需要去日志系统查询具体的慢 SQL 语句是什么。*(注意：这一步是动态决定的，如果上一步发现是内存 OOM，现在的行动就会变成查 Heap Dump)*

1. **行动 (Action)：** 调用日志查询工具 `query_slow_sql(service="user-service", time="09:00-09:30")`

1. **观察 (Observation)：** 获取到慢 SQL：`SELECT * FROM users WHERE age &gt; 20 ORDER BY created_at DESC`，并发现该查询没有命中索引，导致全表扫描。

1. **思考 (Thought)：** 根本原因已经找到（全表扫描导致 CPU 飙升）。接下来我需要找到 `user-service` 的负责人是谁，以便发送报告。

1. **行动 (Action)：** 调用企业通讯录工具 `query_service_owner(service="user-service")`

1. **观察 (Observation)：** 查出负责人是“王建国”，邮箱是 `wangjianguo@company.com`。

1. **思考 (Thought)：** 我现在收集齐了故障原因和联系人信息，可以组织排查报告并发送邮件了。

1. **行动 (Action)：** 调用邮件发送工具 `send_email(to="wangjianguo@company.com", subject="user-service 故障排查", body="发现由于缺少索引导致慢查询...")`

1. **观察 (Observation)：** 返回结果：邮件发送成功。

1. **思考 (Thought)：** 任务已全部闭环完成，可以向用户输出最终答复了。

1. **行动 (Action)：** 结束循环，用自然语言回复用户：“已查明原因是缺少索引导致的慢 SQL，并已向负责人王建国发送了邮件。”
...
1. **实时环境输入（Real-time Environment Input）**：包括 Agent 当前捕获的外部变量，如系统警报信号或用户即时反馈。这些补充数据融入上下文，帮助 LLM 准确评估现状并调整策略。

1. **模型推理模块（LLM Reasoning Module）**：作为 ReAct 的核心引擎，处理逻辑分析与规划。每次迭代中，LLM 整合历史记录、环境输入及任务目标，输出行动方案。

1. **执行工具集与技能库（Tools & Skills）**：充当 Agent 的操作接口，与外部实体互动。其中原子工具（Tools）处理单一操作（如数据库查询、邮件发送）；技能（Skills）则是对多个相关工具的编排封装，提供面向特定业务场景的可复用能力模块（如"故障诊断技能"、"竞品分析技能"）。两者共同构成 Agent 的行动能力边界。

1. **反馈观察机制（Feedback Observation）**：行动完成后，从环境中采集的实际响应，包括成功输出、错误提示或无结果状态。这一信息将被追加至历史上下文中，成为后续推理的可靠基础。
...
**与 ReAct 的对比**
维度ReActPlan-and-Execute规划方式动态、逐步规划静态、全局预规划适用场景动态环境、需实时纠偏步骤明确的长期复杂任务容错能力强（每步可动态修正）弱（环境变化需重新规划）上下文管理随迭代持续增长执行步骤相对独立，更可控
...
## [讲讲 Agent 的反思机制？为什么要用反思？具体怎么实现？](#讲讲-agent-的反思机制-为什么要用反思-具体怎么实现)
...
## [复杂任务怎么做的任务拆分？为什么要拆分？效果如何提升？](#复杂任务怎么做的任务拆分-为什么要拆分-效果如何提升)
...
1. 动态记忆与 RAG：通过滑动窗口管理短期记忆，利用向量数据库检索长期事实，并将外部执行环境的 Observation（如 API 报错日志）进行摘要脱水后实时回传。
...
## [Context Window 管理：短期记忆不够大怎么办？Agent 记忆压缩通常有哪些方法？](#context-window-管理-短期记忆不够大怎么办-agent-记忆压缩通常有哪些方法)
...
![](/imported/markdown/undated-markdown-12063518-agent常见面试题/images/ee5626592e7c-202603102235511.png)
**核心架构模式**
...
维度Single-AgentMulti-Agent（中心化）Multi-Agent（去中心化）架构复杂度低中高Context 压力全部压在一个 Agent各 Agent 独立管理各 Agent 独立管理专业能力泛才，什么都做专才分工，各有专责专才分工，各有专责并行能力不支持支持子任务并行支持并行可控性高高，Orchestrator 统管低，难以统一调度调试难度容易中，按调度链路追踪难，行为不可预测工程实用性高高低，主要用于学术研究适用场景任务清晰、复杂度适中需要分工或并行的复杂任务学术探索场景
具体选型回答：

1. 第一，context 要撑爆了、需要不同专业分工、有子任务可以并行，不属于这三类就用 Single-Agent，盲目引入 Multi-Agent 只会增加系统复杂度，带不来对应收益。

1. 第二，Multi-Agent 架构方案要主动提中心化和去中心化两种，工程里几乎都选 Orchestrator 中心化模式，因为可控、可追踪、出了问题能顺着调度链路排查。

1. 第三，去中心化「听起来灵活」但存在实际问题：任务分配没协调、执行顺序没保证、失败没有感知，这才是它在生产环境里不可用的根本原因。

## [在工程实践中，为什么有时候选择手搓Agent，而不是直接用成熟框架？](#在工程实践中-为什么有时候选择手搓agent-而不是直接用成熟框架)
...
## [什么是 OpenManus？它的实现原理是什么？](#什么是-openmanus-它的实现原理是什么)
...
## [Agent 死循环问题有遇到过吗?如何解决?](#agent-死循环问题有遇到过吗-如何解决)
...
1. 状态变量(如 step=1/2/3)跟踪当前步骤，输入时附带状态标识，模型按状态执行对应动作。

1. 设置调用兜底机制：最多调用3次，并检查返回字段是否变化，检查是否包含 endFlag。

## [什么是 Prompt Injection（提示词注入攻击）？](#什么是-prompt-injection-提示词注入攻击)
...
1. **认知层**：Prompt 隔离与边界划分。区分"System Prompt"和"User Input"。利用大模型 API 原生的 Role 划分机制；拼接外部内容时，使用分隔符将不受信任的数据包裹起来，降低被注入风险。

1. **决策层**：人机协同机制。对于高危工具调用（如修改数据库、发送邮件或转账），不让 Agent 全自动执行。执行前触发工具调用中断，向管理员推送审批请求，拿到授权后继续。
