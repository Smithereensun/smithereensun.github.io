{
  ...
  "has_date": true,
  "description": "线程状态转换 新建(New) NEW：初始状态，线程被构建，但是还没有调用start()方法。 可运行(Runnable) RUNNABLE：可运行状态，可运行状态可以包括：运行中状态和就绪状态。也就是 可能正在运行，也可能正在等待 CPU 时间片。 包含了操作系统线程状态中的 Running 和 ",
  "tags": [
    "Java",
    "并发"
  ],
  "source": "local-markdown-library",
  "source_path": "java/concurrent/01-fundamentalsofconcurrency2-thread - 线程基础的全景图：Java开发者必须掌握的核心知识点.md",
  "date": "2026-02-01"

}

## [线程状态转换](#线程状态转换)
![](/imported/markdown/2026-02-01-markdown-58f70ded-线程基础的全景图-java开发者必须掌握的核心知识点/images/de6a86b94938-202407090028297.png)
### [新建(New)](#新建-new)
...
![](/imported/markdown/2026-02-01-markdown-58f70ded-线程基础的全景图-java开发者必须掌握的核心知识点/images/8c23476e7396-202404251004836.gif)
### [无限期等待(Waiting)](#无限期等待-waiting)

等待其它线程显式地唤醒，否则不会被分配 CPU 时间片。
进入方法退出方法没有设置 Timeout 参数的 Object.wait() 方法Object.notify() / Object.notifyAll()没有设置 Timeout 参数的 Thread.join() 方法被调用的线程执行完毕LockSupport.park() 方法-![](/imported/markdown/2026-02-01-markdown-58f70ded-线程基础的全景图-java开发者必须掌握的核心知识点/images/f8c9da351939-202404251004847.gif)
...
进入方法退出方法Thread.sleep() 方法时间结束设置了 Timeout 参数的 Object.wait() 方法时间结束 / Object.notify() / Object.notifyAll()设置了 Timeout 参数的 Thread.join() 方法时间结束 / 被调用的线程执行完毕LockSupport.parkNanos() 方法-LockSupport.parkUntil() 方法-![](/imported/markdown/2026-02-01-markdown-58f70ded-线程基础的全景图-java开发者必须掌握的核心知识点/images/ce0fd804a70a-202404251004842.gif)
...
1. 用sleep()提供的参数来设置一个定时器

1. 当时间结束定时器会触发，内核收到中断后修改线程的运行状态。就绪队列等待调度例如线程会被标志为就绪而进入就绪队列等待调度

sleep() 可能会抛出 InterruptedException，因为异常不能跨线程传播回 main() 中，因此必须在本地进行处理。线程中抛出的其它异常也同样需要在本地进行处理。

#### [sleep(0)的妙用](#sleep-0-的妙用)
...
![](/imported/markdown/2026-02-01-markdown-58f70ded-线程基础的全景图-java开发者必须掌握的核心知识点/images/f22d1b34bdb1-202404251004858.gif)
...
![](/imported/markdown/2026-02-01-markdown-58f70ded-线程基础的全景图-java开发者必须掌握的核心知识点/images/4712080429c5-202404251004853.gif)
### [interrupted中断](#interrupted中断)
...
输出：
![](/imported/markdown/2026-02-01-markdown-58f70ded-线程基础的全景图-java开发者必须掌握的核心知识点/images/40bba3a69f66-202404251004555.gif)
从上述结果可以看出，线程在接收到中断指令之后，立即中断了线程，相比于上一种自定义中断标识符的方法来说，它能更及时的响应中断线程指令。

### [利用interruptedException](#利用interruptedexception)

这种方式 不 需要在while循环中判断使用

如果线程因为执行join()，sleep或者wait()而进入阻塞状态，此时想要停止它，可以调用interrupt()，程序会抛出interruptedException异常。可以利用这个异常终止线程
![](/imported/markdown/2026-02-01-markdown-58f70ded-线程基础的全景图-java开发者必须掌握的核心知识点/images/b4e815bfb0e7-202404251004577.gif)
...
#### [原理](#原理)
![](/imported/markdown/2026-02-01-markdown-58f70ded-线程基础的全景图-java开发者必须掌握的核心知识点/images/9fdb634a97b7-202404251004601.gif)
...
所以简化一下，流程即：
![](/imported/markdown/2026-02-01-markdown-58f70ded-线程基础的全景图-java开发者必须掌握的核心知识点/images/cfc33ce50953-202404251004626.gif)
...
### [Runnable中异常如何被吞掉](#runnable中异常如何被吞掉)
...
1.
为创建的线程设置一个`UncaughtExceptionHandler`

1.
使用`Callable`代替`Runnable`，`Callable`的`call`方法允许抛出异常，然后可以通过提交给`ExecutorService`返回的`Future`来捕获和处理这些异常

### [Callable中异常如何被吞掉](#callable中异常如何被吞掉)
