{
  ...
  "has_date": true,
  "description": "开篇结论 加锁的对象是**索引**，加锁的基本单位是 next-key lock，它是由记录锁和间隙锁组合而成的，next-key lock 是**左开右闭**区间，而间隙锁是**左开右开**区间。 在**只使用记录锁或者间隙锁**就能避免幻读现象的场景下， next-key lock 就会退化成记",
  "tags": [
    "数据库",
    "MySQL"
  ],
  "source": "local-markdown-library",
  "source_path": "database/mysql/02-lock2-howtoaddrowlocks - Mysql的行级锁到底是怎么加的？.md",
  "date": "2026-04-19"
...
![image-20240426191021264](/imported/markdown/2026-04-19-markdown-05de68bb-mysql的行级锁到底是怎么加的/images/7094171f70a3-202404261918340.png)
...
![](/imported/markdown/2026-04-19-markdown-05de68bb-mysql的行级锁到底是怎么加的/images/61465fda4855-202404261918052.png)
...
![](/imported/markdown/2026-04-19-markdown-05de68bb-mysql的行级锁到底是怎么加的/images/55851070eedd-202404261918668.png)
...
### [针对 大于或者大于等于 的范围查询](#针对-大于或者大于等于-的范围查询)

#### [针对 大于 的范围查询的情况](#针对-大于-的范围查询的情况)

假设事务 A 执行了这条范围查询语句：

此时，事务 A 在主键索引上加了两个next-key 锁：
![](/imported/markdown/2026-04-19-markdown-05de68bb-mysql的行级锁到底是怎么加的/images/afa770223be7-202404261918506.png)
...
#### [针对 大于等于 的范围查询的情况。](#针对-大于等于-的范围查询的情况。)

假设事务 A 执行了这条范围查询语句：

此时事务 A 在主键索引上加了三个 X 型 的锁，分别是：
![](/imported/markdown/2026-04-19-markdown-05de68bb-mysql的行级锁到底是怎么加的/images/4dacb3c407e6-202404261918375.png)
...
### [针对 小于或者小于等于 的范围查询](#针对-小于或者小于等于-的范围查询)

#### [针对 小于/小于等于 的范围查询时，查询条件值的记录 不存在 表中的情况](#针对-小于-小于等于-的范围查询时-查询条件值的记录-不存在-表中的情况)

假设事务 A 执行了这条范围查询语句，注意查询条件值的记录（id 为 6）并不存在于表中。

此时，事务 A 在主键索引上加了三个 X 型的锁：
![](/imported/markdown/2026-04-19-markdown-05de68bb-mysql的行级锁到底是怎么加的/images/54b3bfadb427-202404261918401.png)
...
#### [针对 小于等于 的范围查询时，查询条件值的记录 存在 表中的情况。](#针对-小于等于-的范围查询时-查询条件值的记录-存在-表中的情况。)

假设事务 A 执行了这条范围查询语句，注意查询条件值的记录（id 为 5）存在于表中。

此时，事务 A 在主键索引上加了 2 个 X 型的锁：
![](/imported/markdown/2026-04-19-markdown-05de68bb-mysql的行级锁到底是怎么加的/images/5a3a2e72b021-202404261918129.png)
...
#### [针对 小于 的范围查询时，查询条件值的记录 存在 表中的情况。](#针对-小于-的范围查询时-查询条件值的记录-存在-表中的情况。)

假设事务 A 执行了这条范围查询语句，注意查询条件值的记录（id 为 10）并不存在于表中。

此时，事务 A 在主键索引上加了三个 X 型的锁：
![](/imported/markdown/2026-04-19-markdown-05de68bb-mysql的行级锁到底是怎么加的/images/b8e7e10b3f38-202404261918550.png)
...
![](/imported/markdown/2026-04-19-markdown-05de68bb-mysql的行级锁到底是怎么加的/images/3a937cfb4038-202404261919384.png)
...
![](/imported/markdown/2026-04-19-markdown-05de68bb-mysql的行级锁到底是怎么加的/images/436e23f804e2-202404261919021.png)
...
#### [插入age = 22 和 age = 21的情况](#插入age-22-和-age-21的情况)
...
此时，事务 A 对主键索引和二级索引都加了 X 型的锁：
![](/imported/markdown/2026-04-19-markdown-05de68bb-mysql的行级锁到底是怎么加的/images/4bd2e9a647f1-202404261919849.png)
...
这里说的是，当查询无法利用任何索引进行过滤时，InnoDB 只能从头到尾扫描整个主键索引树，也就是加到了主键索引上，也就是相当于锁住的全表
