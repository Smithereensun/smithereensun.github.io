{
  ...
  "title": "TCP - 三次握手四次挥手",
  "has_date": true,
  "description": "TCP和UDP的区别 TCP是面向字节流、可靠、面向连接； UDP是面向数据报文段、不可靠、无连接； 可靠和不可靠： TCP通过一系列机制保证可靠性，而UDP是不可靠的 面向连接和无连接：TCP是面向连接的，通过连接管理机制，也就是三次握手和四次挥手实现。 UDP是面向数据包的 当用户消息通过 UD",
  "tags": [
    "计算机基础"
  ],
  "source": "local-markdown-library",
  "source_path": "cs-basics/network/02-tcp1-threehandshakesandfourwaves - TCP - 三次握手四次挥手.md",
  "date": "2025-09-21"
...
### [建立过程](#建立过程)
![](/imported/markdown/2025-09-21-markdown-627fcd16-tcp-三次握手四次挥手/images/8f128a33e8e8-202404271112161.png)
...
#### [避免历史连接(为什么不能2次)](#避免历史连接-为什么不能2次)

![](/imported/markdown/2025-09-21-markdown-627fcd16-tcp-三次握手四次挥手/images/9216abb539c5-202404271113164.png)
...
1. 客户端收到后，发现自己期望收到的确认号应该是 100 + 1，而不是 90 + 1，于是就会回 RST 报文。

1. 服务端收到 RST 报文后，就会释放连接。

1. 后续最新的 SYN 抵达了服务端后，客户端与服务端就可以正常的完成三次握手了。
...
### [每次建立 TCP 连接时，初始化的序列号都要求不一样](#每次建立-tcp-连接时-初始化的序列号都要求不一样)
...
![](/imported/markdown/2025-09-21-markdown-627fcd16-tcp-三次握手四次挥手/images/fe44130269d5-202404271113313.png)
 假设每次建立连接，客户端和服务端的初始化序列号都是从 0 开始：

1. 客户端和服务端建立一个 TCP 连接，在客户端发送数据包被网络阻塞了，然后超时重传了这个数据包，而此时服务端设备断电重启了，之前与客户端建立的连接就消失了，于是在收到客户端的数据包的时候就会发送 RST 报文。

1. 紧接着，客户端又与服务端建立了与上一个连接相同四元组的连接；

1. 在新连接建立完成后，上一个连接中被网络阻塞的数据包正好抵达了服务端，刚好该数据包的序列号正好是在服务端的接收窗口内，所以该数据包会被服务端正常接收，就会造成数据错乱。
...
1. **服务端**就收不到第三次握手，于是服务端这边会触发超时重传机制，重传 SYN-ACK 报文。
...
![](/imported/markdown/2025-09-21-markdown-627fcd16-tcp-三次握手四次挥手/images/76ab46f2af89-202404271113095.png)
...
1. 开启 tcp_syncookies；开启 syncookies 功能就可以在不使用 SYN 半连接队列的情况下成功建立连接，相当于绕过了 SYN 半连接来建立连接。

1. 减少 SYN+ACK 重传次数。当服务端受到 SYN 攻击时，就会有大量处于 SYN_REVC 状态的 TCP 连接，处于这个状态的 TCP 会重传 SYN+ACK，当重传超过次数达到上限后，就会断开连接。

## [TCP四次挥手](#tcp四次挥手)

### [挥手过程](#挥手过程)
![](/imported/markdown/2025-09-21-markdown-627fcd16-tcp-三次握手四次挥手/images/a7553989680c-202404271113235.png)
...
1. 主机1在收到第三次挥手后，就会进入 TIME_WAIT 状态，开启时长为 2MSL 的定时器，如果途中再次收到第三次挥手（FIN 报文）后，就会重置定时器，当等待 2MSL 时长后，主机1就会断开连接。

### [TIME_WAIT状态](#time-wait状态)
...
### [如果双方同时发起断开，会发生什么](#如果双方同时发起断开-会发生什么)
...
#### [HTTP 长连接的请求数量达到上限](#http-长连接的请求数量达到上限)
...
### [服务器出现大量 CLOSE_WAIT 状态的原因](#服务器出现大量-close-wait-状态的原因)
...
## [TCP连接中发送RST的五种情况](#tcp连接中发送rst的五种情况)

1. 客户端尝试与服务器未对外提供服务的端口建立TCP连接，服务器将会直接向客户端发送reset报文。

1. 客户端和服务器的某一方在交互的过程中发生异常（如程序崩溃等），该方系统将向对端发送TCP reset报文，告之对方释放相关的TCP连接。

1. 接收端收到TCP报文，但是发现该TCP的报文，并不在其已建立的TCP连接列表内，则其直接向对端发送reset报文。

1. 在交互的双方中的某一方长期未收到来自对方的确认报文，则其在超出一定的重传次数或时间后，会主动向对端发送reset报文释放该TCP连接。

1. 有些应用开发者在设计应用系统时，会利用reset报文快速释放已经完成数据交互的TCP连接，以提高业务交互的效率。
