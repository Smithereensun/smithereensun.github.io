{
  ...
  "has_date": true,
  "description": "导言 Redis是一个高效的内存数据库，它支持包括String、List、Set、SortedSet和Hash等数据类型的存储，在Redis中通常根据数据的key查询其value值，Redis没有模糊条件查询，在面对一些需要分页、排序以及条件查询的场景时(如评论，时间线，检索等)，只凭借Redis所",
  "tags": [
    "数据库",
    "Redis"
  ],
  "source": "local-markdown-library",
  "source_path": "database/redis/07-practice-redis-pagination-multi-condition-fuzzy-query - Redis实现分页+多条件模糊查询组合方案.md",
  "date": "2025-10-26"
...
![](/imported/markdown/2025-10-26-markdown-f1c2aef9-redis实现分页-多条件模糊查询组合方案/images/150bf6929c30-202502142109399.webp)
...
## [Redis的多条件模糊查询实现](#redis的多条件模糊查询实现)
...
**查询所有性别为女的用户**
![](/imported/markdown/2025-10-26-markdown-f1c2aef9-redis实现分页-多条件模糊查询组合方案/images/4d2f28a7efa8-202502142109403.webp)
**查询所有名字中姓阿的用户**
![](/imported/markdown/2025-10-26-markdown-f1c2aef9-redis实现分页-多条件模糊查询组合方案/images/a14e527d6a26-202502142109422.webp)
HSCAN虽然为我们提供了模式匹配的功能，但这种匹配是基于遍历实现的，每一次匹配都需要遍历全部的key，效率上并不高。因此在下面一节会这方面进行补充，本节只谈如何实现模糊匹配。

## [Redis的分页+多条件模糊查询组合实现](#redis的分页-多条件模糊查询组合实现)
...
![](/imported/markdown/2025-10-26-markdown-f1c2aef9-redis实现分页-多条件模糊查询组合方案/images/a90601c10489-202502142109427.webp)
