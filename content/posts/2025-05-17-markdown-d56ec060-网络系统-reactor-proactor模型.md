{
  ...
  "has_date": true,
  "description": "演进 如果要让服务器服务多个客户端，那么最直接的方式就是为每一条连接创建线程。 其实创建进程也是可以的，原理是一样的，进程和线程的区别在于线程比较轻量级些，线程的创建和线程间切换的成本要小些，为了描述简述，后面都以线程为例。处理完业务逻辑后，随着连接关闭后线程也同样要销毁了，但是这样不停地创建和销毁",
  "tags": [
    "计算机基础"
  ],
  "source": "local-markdown-library",
  "source_path": "cs-basics/operating-system/reactor-proactor - 网络系统 - Reactor & Proactor模型.md",
  "date": "2025-05-17"
...
![](/imported/markdown/2025-05-17-markdown-d56ec060-网络系统-reactor-proactor模型/images/312cd0aa78a4-202406222123181.png)
...
大佬们还为这种模式取了个让人第一时间难以理解的名字：**Reactor 模式**。Reactor 翻译过来的意思是「反应堆」，可能大家会联想到物理学里的核反应堆，实际上并不是的这个意思。这里的反应指的是「**对事件反应**」，也就是**来了一个事件，Reactor 就有相对应的反应/响应**。
...
![](/imported/markdown/2025-05-17-markdown-d56ec060-网络系统-reactor-proactor模型/images/97176f7fc268-202406222123930.png)
...
![](/imported/markdown/2025-05-17-markdown-d56ec060-网络系统-reactor-proactor模型/images/f4c7b559a399-202406222123192.png)
详细说一下这个方案：
...
单 Reator 多线程的方案优势在于**能够充分利用多核 CPU 的能力**，那既然引入多线程，那么自然就带来了多线程竞争资源的问题。例如，子线程完成业务处理后，要把结果传递给主线程的 Handler 进行发送，这里涉及共享数据的竞争。
...
![](/imported/markdown/2025-05-17-markdown-d56ec060-网络系统-reactor-proactor模型/images/49df1f71e028-202406222123233.png)
方案详细说明如下：
...
### [Reactor模型示例（Java实现）](#reactor模型示例-java实现)
...
![阻塞 I/O](/imported/markdown/2025-05-17-markdown-d56ec060-网络系统-reactor-proactor模型/images/9b4e5e7a62e6-202406222123178.png)阻塞 I/O
**非阻塞 I/O**，非阻塞的 read 请求在数据未准备好的情况下立即返回，可以继续往下执行，此时应用程序不断轮询内核，直到数据准备好，内核将数据拷贝到应用程序缓冲区，`read` 调用才可以获取到结果。过程如下图：
![非阻塞 I/O](/imported/markdown/2025-05-17-markdown-d56ec060-网络系统-reactor-proactor模型/images/4435c54ec56b-202406222123168.png)非阻塞 I/O
...
![异步 I/O](/imported/markdown/2025-05-17-markdown-d56ec060-网络系统-reactor-proactor模型/images/152fdb1d7eea-202406222123204.png)异步 I/O
...
Proactor 模式的示意图：
![](/imported/markdown/2025-05-17-markdown-d56ec060-网络系统-reactor-proactor模型/images/03ca180d0441-202406222123940.png)
Proactor 模式的工作流程：
