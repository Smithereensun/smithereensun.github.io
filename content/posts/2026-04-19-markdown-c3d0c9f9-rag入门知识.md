{
  ...
  "title": "Rag入门知识",
  "has_date": true,
  "description": "什么是Rag？ RAG，Retrieval-Augmented Generation，中文名检索增强生成，是AI领域非常重要的一种技术方案。其核心作用是给LLM大模型外挂专门的知识库，指导大模型生成更准确的输出。 为什么要给LLM大模型外挂知识库呢？因为虽然大模型的能力越来越强大，但其内在的缺点也非",
  "tags": [
    "AI",
    "RAG"
  ],
  "source": "local-markdown-library",
  "source_path": "ai/rag-summary - Rag入门知识.md",
  "date": "2026-04-19"
...
## [Rag技术的基本原理](#rag技术的基本原理)
![](/imported/markdown/2026-04-19-markdown-c3d0c9f9-rag入门知识/images/9c8ec87d003c-202506011831240.jpg)
...
1. **智能客服系统**：RAG可以结合产品资料知识库、聊天记录、用户反馈等数据，自动为用户提供更精准的回答，已经有非常多的初创公司选择用RAG技术构建新一代的智能客服系统。

1. **智能数据分析**：RAG可以结合外部数据源，如数据库、API、文件等，为用户提供更便捷的数据分析服务。传统企业的数据分析主要靠BI分析师，每天都需要写大量的SQL语句进行查询，而在RAG的支持下，企业的每个员工都能以自然对话的方式获取数据。比如门店店长直接用语音对话，“请帮我找出上周销量排名前10，但本周销量下滑最快的品类”，系统即可直接给出答复。

1. **自动化文档处理**：企业还可以利用RAG和LLM大模型自动化文档处理流程，例如自动生成合同、撰写周报、总结会议纪要等，节省时间和人力成本。
...
### [第一阶段，可运行，即系统能跑通整体流程](#第一阶段-可运行-即系统能跑通整体流程)

#### [1）知识分块与索引](#_1-知识分块与索引)
...
### [第二阶段，可使用，即系统初步达到可上线水平](#第二阶段-可使用-即系统初步达到可上线水平)

#### [1）知识分块与索引](#_1-知识分块与索引-1)
...
**索引降噪**：是根据业务特点，去除索引数据中的无效成分，突出其核心知识，从而降低噪音的干扰，保障核心知识的比重。比如原文档内容是“How can I download source code from [github.com](http://github.com)”，其核心内容是“download source code、github”，其他噪音可以忽略。
...
### [第三阶段，很好用，即系统回答的准确率达到用户满意水平](#第三阶段-很好用-即系统回答的准确率达到用户满意水平)

#### [1）知识分块与索引](#_1-知识分块与索引-2)
...
## [Rag系统的发展历程，从朴素、高级到模块化](#rag系统的发展历程-从朴素、高级到模块化)
...
1. 检索：对用户输入的Query问题，使用相同的embedding模型，计算Query嵌入和文档块嵌入之间的向量相似度，然后选择相似度最高的前N个文档块作为当前问题的增强上下文信息。

1. 生成：将原始Query和相关文档合并为新的提示，然后由大型语言模型基于提供的信息回答问题。如果有历史对话信息，也可以合并到提示中，用于进行多轮对话。
...
#### [Pre-Retrieval 检索前](#pre-retrieval-检索前)
...
- 索引降噪：是根据业务特点，去除索引数据中的无效成分，突出其核心知识，降低噪音的干扰。比如原文档内容是“How can I download source code from [github.com](http://github.com)”，其核心内容是“download source code、github”，其他噪音可以忽略。
...
#### [Post-Retrieval 检索后](#post-retrieval-检索后)
...
1. 增加元数据：块可以用元数据（比如页码、文件名、作者、时间戳、摘要或问题）进行丰富，让索引更加精准。这个优化方案在高级Rag中即有使用，被继承到了模块化Rag中。

1. 从小到大：将用于检索的块和用于合成的块区分开，在检索的时候用较小的块，在合成的时候用较大的块。
...
1. 密集检索器：使用预训练的语言模型（PLMs）为查询和文档提供密集表示。尽管计算和存储成本较高，却能提供更复杂的语义表示
