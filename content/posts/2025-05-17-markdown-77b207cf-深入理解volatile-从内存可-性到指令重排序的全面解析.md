{
  ...
  "has_date": true,
  "description": "介绍 volatile是轻量级的同步机制，volatile可以用来解决可见性和有序性问题，但不保证原子性。 volatile的作用： 保证了不同线程对共享变量进行操作时的可见性，即一个线程修改了某个变量的值，这新值对其他线程来说是立即可见的。 禁止进行指令重排序。 底层原理 内存屏障 volatil",
  "tags": [
    "Java",
    "并发"
  ],
  "source": "local-markdown-library",
  "source_path": "java/concurrent/02-keyword2-volatile - 深入理解volatile：从内存可⻅性到指令重排序的全面解析.md",
  "date": "2025-05-17"
...
1. 每个volatile写后插入StoreLoad屏障；防止volatile写与之后可能有的volatile读/写重排序；

1. 每个volatile读后插入LoadLoad屏障；禁止之后所有的普通读操作和volatile读操作重排序；

1. 每个volatile读后插入LoadStore屏障。禁止之后所有的普通写操作和volatile读重排序；
...
![](/imported/markdown/2025-05-17-markdown-77b207cf-深入理解volatile-从内存可-性到指令重排序的全面解析/images/c0745fd5ccd9-202404251029175.gif)
...
![](/imported/markdown/2025-05-17-markdown-77b207cf-深入理解volatile-从内存可-性到指令重排序的全面解析/images/9e6a5baf82a8-202404251029184.jpg)
...
![](/imported/markdown/2025-05-17-markdown-77b207cf-深入理解volatile-从内存可-性到指令重排序的全面解析/images/dccafba9baa4-202404251029181.jpg)
...
![](/imported/markdown/2025-05-17-markdown-77b207cf-深入理解volatile-从内存可-性到指令重排序的全面解析/images/bf9473f8ebbe-202404251029182.jpg)![](/imported/markdown/2025-05-17-markdown-77b207cf-深入理解volatile-从内存可-性到指令重排序的全面解析/images/5d6f6c5d0c6f-202404251029178.jpg)
### [为什么不能保证原子性](#为什么不能保证原子性)
...
1. 线程B也读取count的当前值为0（在线程A增加count之前）。
