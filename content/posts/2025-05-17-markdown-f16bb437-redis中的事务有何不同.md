{
  ...
  "title": "Redis中的事务有何不同？",
  "has_date": true,
  "description": "与关系型数据库事务的区别 Redis事务是指将多条命令加入队列，一次批量执行多条命令，每条命令会按顺序执行，事务执行过程中不会被其他客户端发来的命令所打断。也就是说，Redis事务就是一次性、顺序性、排他性的执行一个队列中的一系列命令。 Redis事务和关系型数据库的事务不太一样，它**不保证原子性",
  "tags": [
    "数据库",
    "Redis"
  ],
  "source": "local-markdown-library",
  "source_path": "database/redis/05-transactionofredis - Redis中的事务有何不同？.md",
  "date": "2025-05-17"

}

## [与关系型数据库事务的区别](#与关系型数据库事务的区别)
...
![](/imported/markdown/2025-05-17-markdown-f16bb437-redis中的事务有何不同/images/c611ba0f45d6-202404270809647.png)

### [事务不保证原子性](#事务不保证原子性)

如上图所示，在通过EXEC执行事务时，其中命令执行失败不会影响到其他命令的执行，因此并没有保证同时成功和同时失败的原子操作，尽管这样，Redis事务中也**没有提供回滚**的支持
![](/imported/markdown/2025-05-17-markdown-f16bb437-redis中的事务有何不同/images/fe7b10a9ff18-202404270810674.png)
...
#### [语法错误（编译器错误）](#语法错误-编译器错误)

在开启事务后，A的转出操作命令打成了`DECRBYa`，最终会导致事务提交失败，所有命令都不会执行，A、B保留原值。

#### [类型错误（运行时错误）](#类型错误-运行时错误)

在运行时检测类型错误，此时事务并没有回滚，而是跳过错误命令继续执行， 结果B值改变、A保留原值。
![](/imported/markdown/2025-05-17-markdown-f16bb437-redis中的事务有何不同/images/2381fec02896-202404270810644.png)
...
## [使用WATCH实现乐观锁](#使用watch实现乐观锁)

WATCH通过监视指定Redis Key，如果没有改变，就执行成功，如果发现对应值发生改变，事务就会执行失败，如下图：
![](/imported/markdown/2025-05-17-markdown-f16bb437-redis中的事务有何不同/images/755d9f34f94f-202404270810967.png)
