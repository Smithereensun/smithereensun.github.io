...
　　将logback-spring.xml配置文件，放到：/src/main/resources中
...
<configuration  scan="true" scanPeriod="10 seconds">
    ...
    <property name="log.name" value="online_ybclass"/>
...
![](/imported/posts/2020-08-15-13507486-6e6a6862-springboot-日志文件-logback-spring-xml/images/img_001_23263f1bc602.png)

# 日志输出效果

![](/imported/posts/2020-08-15-13507486-6e6a6862-springboot-日志文件-logback-spring-xml/images/img_002_0ea07aa47c54.png)
...
<configuration  scan="true" scanPeriod="10 seconds">
    ...
    <!-- name的值是变量的名称，value的值时变量定义的值。通过定义的值会被插入到logger上下文中。定义变量后，可以使“${}”来使用变量。 -->
    <property name="log.path" value="applog/" />
    <property name="log.name" value="ecomm-service"/>
    <property name="CONSOLE_LOG_PATTERN_FILE" value="%d{yyyy-MM-dd HH:mm:ss.SSS} %C:%M:%L [%thread] %-5level %msg%n"/>
    ...
    <appender name="WARN_FILE" class="ch.qos.logback.core.rolling.RollingFileAppender">
        <!-- 正在记录的日志文件的路径及文件名 -->
        <file>${log.path}/${log.name}/${log.name}_warn.log</file>
        <!--日志文件输出格式-->
        <encoder>
            <pattern>${CONSOLE_LOG_PATTERN_FILE}</pattern>
            <charset>UTF-8</charset> <!-- 此处设置字符集 -->
        ...
        <!-- 日志记录器的滚动策略，按日期，按大小记录 -->
        <rollingPolicy class="ch.qos.logback.core.rolling.TimeBasedRollingPolicy">
            <fileNamePattern>${log.path}/${log.name}/warn/${log.name}-warn-%d{yyyy-MM-dd}.%i.log</fileNamePattern>
            <timeBasedFileNamingAndTriggeringPolicy class="ch.qos.logback.core.rolling.SizeAndTimeBasedFNATP">
                <maxFileSize>100MB</maxFileSize>
            </timeBasedFileNamingAndTriggeringPolicy>
            <!--日志文件保留天数-->
