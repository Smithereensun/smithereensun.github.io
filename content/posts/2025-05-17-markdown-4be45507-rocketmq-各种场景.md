{
  ...
  "title": "RocketMQ - 各种场景",
  "has_date": true,
  "description": "RocketMQ的基本原理 RocketMQ基本架构图如下** 从这个架构图上我们可以知道，RocketMQ有4块核心部分： NameServer：管理Broker的信息，让使用MQ的系统感知到集群里面的broker Broker：主从架构实现数据多副本存储和高可用 producer：生产者 con",
  "tags": [
    "微服务",
    "消息队列",
    "RocketMQ"
  ...
  "source_path": "microservices/message-queue/rocketMQ-scenarios - RocketMQ - 各种场景.md",
  "date": "2025-05-17"

}

## [RocketMQ的基本原理](#rocketmq的基本原理)

**RocketMQ基本架构图如下**
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/48c50dc58ffa-202405030905786.png)
...
### [Broker信息注册到哪个NameServer？](#broker信息注册到哪个nameserver)

每台broker机器需要向所有的NameServer机器上注册自己的信息，防止单台NameServer挂掉导致Broker信息不全，保证NameServer的集群高可用。

### [Broker信息怎么注册？](#broker信息怎么注册)

基于Netty的网络通信。

### [Broker挂了如何感知？](#broker挂了如何感知)

- NameServer感知：30s心跳机制和120s故障感知机制

![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/46043565c335-202405030905369.png)
...
### [Producer是如何选择Broker？](#producer是如何选择broker)
...
### [如果某个 Broker 在 Producer 准备写入数据的时候突然宕机了，如何处理呢？](#如果某个-broker-在-producer-准备写入数据的时候突然宕机了-如何处理呢)
...
### [Master-Slave模式](#master-slave模式)

为了保证MQ的数据不丢失而且具备一定的高可用性，我们采用的是主从复制模式。
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/6855bd27bd56-202405030905585.png)
...
#### [Pull方式](#pull方式)
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/6df5ae0039ac-202405030906054.png)
详细介绍pull模式的数据拉取过程：
...
1. 在完成连接后，从节点会创建两个线程，也就是两个HAClient：一个负责处理主从同步的请求，一个负责处理主从同步的响应。

1. HAClient创建成功后，每经过一段时间，就会发出pull请求，请求同步主节点的数据。需要同步的数据的范围是指定在CommitLog中以特定物理偏移量（Offset）开始的所有消息。

1. 收到请求后，主节点会将offset指定位置之后的所有数据发回给从节点。从节点在接收到这些数据后，会由HAClient的响应线程将这些数据写入本地的CommitLog。

1. 然而，当消息被写入到pageCache且当前服务器突然宕机，该部分数据还未得以同步到从节点。由于pageCache是由操作系统管理，这就导致了在切换到从节点时，这部分数据将被丢失。

**那么，我们应该如何预防数据在同步过程中丢失呢？**

**主从同步过程-数据0丢失**
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/8b115d7fe965-202405030906612.png)
...
1. 一旦主节点接收到此请求，它会通过查询和对比本地磁盘的offset，需进行一次磁盘IO。然后，主节点将最新的数据发送给从节点，包含一次网络IO。消息到达从节点之后，进行一次磁盘IO写入操作。
...
#### [push方式](#push方式)
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/4fac8b7f7544-202405030906033.png)
push模式的主从同步过程：
...
### [如何从Master-Slave中进行读写呢？](#如何从master-slave中进行读写呢)
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/efcb706c6847-202405030906692.png)
...
选举方式这里不做重点介绍。
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/15783efe3737-202405030907919.png)
## [生产者](#生产者)

### [MessageQueue是什么？](#messagequeue是什么)

先看看Topic、Broker、Message之间的关系。

如图比如说一个TopicA有n条消息，然后一个TopicA中的n条数据分配放入给4个MessageQueue1-4。
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/00fc1ef02555-202405030907401.png)
**所以本质上来说就是一个数据分片机制，通过MessageQueue将一个Topic的数据拆分为很多数据分片，在每个Broker机器上都存储一些MessageQueue。通过这个方法可以实现分布式存储。**

### [生产者发送消息写入哪个MessageQueue？](#生产者发送消息写入哪个messagequeue)
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/be7eaa6e65d4-202405030907520.png)
...
#### [Master Broker故障分析](#master-broker故障分析)
...
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/51730f7db5d6-202405030907459.png)
### [Broker数据存储（核心环节）](#broker数据存储-核心环节)
...
#### [磁盘日志文件CommitLog](#磁盘日志文件commitlog)
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/e209d1db26cb-202405030907893.png)
...
#### [ConsumeQueue存储对应消息的偏移量](#consumequeue存储对应消息的偏移量)

**在Broker中，每一个Topic下的每一个MessageQueue都会有对应一系列的ConsumeQueue文件。**

**Broker磁盘存储类似于文件树的形式存在：**
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/37902e30d14b-202405030908701.png)
**ConsumeQueue中存储着对应MessageQueue中的消息在CommitLog中的物理偏移量地址offset。**
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/f037bb9e89c2-202405030908026.png)
**如图：**

1. Broker接受消息，顺序写入消息到CommitLog中

1. 同时找到对应的TopicA/MessageQueue1/ConsumeQueue0写入对应的物理地址

1. TopicA/MessageQueue1/ConsumeQueue0的物理地址，即为CommitLog文件中一个消息的引用

即：**Topic的每个MessageQueue都对应了Broker机器上的多个ConsumeQueue文件，这些ConsumeQueue共同组成保存了MessageQueue的所有消息在CommitLog文件中的物理offset偏移量。**

#### [Broker写入磁盘CommitLog怎么近乎内存写性能？](#broker写入磁盘commitlog怎么近乎内存写性能)

**磁盘文件顺序写+OS PageCache写入+OS异步刷盘的策略**
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/33178f897b2a-202405030908547.png)
**如图：**

1. 数据写入CommitLog时候，不是直接写入磁盘，而是写入OS的PageCache内存缓冲区中

1. 后台开启线程，异步刷盘到CommitLog中
...
#### [一个Topic上多个MessageQueue怎么被消费?](#一个topic上多个messagequeue怎么被消费)

**原则**：集群模式下，一个Consumer机器可以消费处理多个MessageQueue，一个MessageQueue只能被一个相同ConsumerGroup中的同一个Consumer消费。
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/b92f2fc982c5-202405030908976.png)
...
#### [Broker收到消息拉取请求，返回给消费者处理提交消费进度](#broker收到消息拉取请求-返回给消费者处理提交消费进度)

Broker收到消息拉取请求后，会找到对应的MessageQueue中开始消费的位置，在ConsumeQueue读取里面对应位置的的消息在CommitLog中的offset
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/8cf2dab48b55-202405030909097.png)
**如图：**

1. consumer找到要消费的MessageQueue对应的ConsumeQueue对应要消费的位置

1. 消费完成之后消费者返回一个消费状态，broker会存储消费位置

1. 接下来可以根据这个消费位置进行下一步消费，不需要从头拉取

#### [消费者消费消息的性能问题](#消费者消费消息的性能问题)
...
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/594963c2dba1-202405030909579.png)
...
#### [Master Broker什么时候通知你去Slave Broker读取？](#master-broker什么时候通知你去slave-broker读取)
...
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/f1024faaec87-202405030909878.png)

1. 用户进行一笔生活缴费

1. 订单系统推送缴费订单支付消息到RocketMQ
...
#### [系统推送消息丢失](#系统推送消息丢失)
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/22a6666c269a-202405030909439.png)
第一个环节就是，订单系统推送消息到MQ的过程中，由于网络等因素导致消息丢失。

#### [RocketMQ的事务消息原理分析](#rocketmq的事务消息原理分析)

为了解决系统推送消息丢失问题，RocketMQ有一个非常强悍的功能就是事务消息，能够确保消息一定会成功写入MQ里面，不会半路搞丢。

如图是在本系统中的一个基本事务消息的流程图。
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/0253014d6d75-202405030910504.png)
...
#### [RocketMQ的事务消息底层分析](#rocketmq的事务消息底层分析)
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/7102b0c5991c-202405030910152.png)
**如图解释如下：**
...
**如图示意：**
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/c3069907a46a-202405030911132.png)
**解决**：
...
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/9fc662e23e58-202405030911881.png)
...
#### [消费系统拿到了消息就一定会消费消息吗？](#消费系统拿到了消息就一定会消费消息吗)
...
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/e4918459a434-202405030911796.png)
...
### [消息发送全链路零丢失方案总结](#消息发送全链路零丢失方案总结)

#### [发送消息到MQ的零丢失](#发送消息到mq的零丢失)

- 同步发送消息+反复多次尝试

- 事务消息机制

#### [MQ收到消息之后的零丢失](#mq收到消息之后的零丢失)
...
**如图：**
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/a57f49088a67-202405030911207.png)
...
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/16d345213635-202405030912108.png)

1. 由于订单系统调用超时，重试调用接口

1. 当订单系统发消息之前，发送请求到MQ查询是否存在这条消息

1. 如果MQ已经存在，则不重复发送

- Redis缓存：

![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/56580dcec556-202405030912226.png)
...
#### [消费方重复消费](#消费方重复消费)
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/b81f1dbbf8e0-202405030912492.png)
...
如图：
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/5f6ae9ab712e-202405030912045.png)
**问题：**
...
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/9eed4a580795-202405030912867.png)
...
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/e067cf15819b-202405030912811.png)
#### [乱序问题原理分析](#乱序问题原理分析)
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/bdd467b30088-202405030913835.png)
...
**如图：**
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/3f4f90e6b3ee-202405030913139.png)
...
**如图：**
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/03d5ae56a3e5-202405030913223.png)
...
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/b4c254ec31ec-202405030913755.png)
### [解决方案](#解决方案)
...
**如图：**
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/d4a5ae701def-202405030914625.png)
**MessageQueue数量少：**
...
**如图：**
![](/imported/markdown/2025-05-17-markdown-4be45507-rocketmq-各种场景/images/06eafd1bee4f-202405030914573.png)
## [MQ集群数据迁移问题：双读+双写](#mq集群数据迁移问题-双读-双写)
