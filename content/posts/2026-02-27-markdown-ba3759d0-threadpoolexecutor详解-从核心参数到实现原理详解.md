{
  ...
  "has_date": true,
  "description": "频繁创建新线程的缺点？ 不受控风险 系统资源有限，每个人针对不同业务都可以手动创建线程，并且创建标准不一样（比如线程没有名字）。当系统运行起来，所有线程都在疯狂抢占资源，毫无规则，不好管控。 另外，过多的线程自然也会引起上下文切换的开销。 频繁创建开销大 new Thread() 在操作系统层面并没",
  "tags": [
    "Java",
    "并发"
  ],
  "source": "local-markdown-library",
  "source_path": "java/concurrent/04-threadpool1-threadpoolexecutor - ThreadPoolExecutor详解：从核心参数到实现原理详解.md",
  "date": "2026-02-27"

}

## [频繁创建新线程的缺点？](#频繁创建新线程的缺点)
...
1. 一些支持本机方法的 jvm 也会分配一个本机堆栈

1. 每个线程获得一个程序计数器，告诉它当前处理器执行的指令是什么
...
- 控制并发执行个数：池子里有有限个数的worker线程，一直在做无限循环不结束，每循环一次就从阻塞队列里取一个任务调run方法
...
### [线程池的核心组成](#线程池的核心组成)
![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/bc1c8921c42f-202404251117645.png)
...
### [线程池执行具体过程](#线程池执行具体过程)
![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/a84f0fa5d1e2-202404251113478.gif)
...
1. 当线程池里面存活的线程数已经等于corePoolSize了，并且任务队列也满了，假设maximumPoolSize&gt;corePoolSize，这时如果再来新的任务，线程池就会继续创建新的线程来处理新的任务，直到线程数达到maximumPoolSize，就不会再创建了。

1. 如果当前的线程数达到了maximumPoolSize，并且任务队列也满了，如果还有新的任务过来，那就直接采用拒绝策略进行处理。默认的拒绝策略是抛出一个RejectedExecutionException异常。

### [线程池生命周期](#线程池生命周期)
线程池状态状态释义RUNNING线程池被创建后的初始状态，能接受新提交的任务，并且也能处理阻塞队列中的任务SHUTDOWN关闭状态，不再接受新提交的任务，但仍可以继续处理已进入阻塞队列中的任务STOP会中断正在处理任务的线程，不能再接受新任务，也不继续处理队列中的任务TIDYING所有的任务都已终止，workerCount(有效工作线程数)为0TERMINATED线程池彻底终止运行
Tips：千万不要把线程池的状态和线程的状态弄混了。补一张网上的线程状态图
![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/b62c412fd742-202404251117843.png)
当线程调用start()，线程在JVM中不一定立即执行，有可能要等待操作系统分配资源，此时为READY状态，当线程获得资源时进入RUNNING状态，才会真正开始执行。

### [线程池的预初始化机制](#线程池的预初始化机制)
...
![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/fbcf7058f15a-202404251117769.png)![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/e9df505e28f8-202404251117174.png)
...
1. 继承`BlockingQueue`实现一个混合式阻塞队列，该队列包含`JDK`自带的`ArrayBlockingQueue`。另外，该混合式阻塞队列需要修改取任务处理的逻辑，也就是重写`take()`方法，取任务时优先从数据库中读取最早的任务，数据库中无任务时再从 `ArrayBlockingQueue`中去取任务。

![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/579da6c64f24-202405241916054.webp)
...
![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/9f2ed2f12936-202404251118495.png)
...
![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/6f5ad778b493-202404251118645.png)![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/1b805217c79f-202404251118194.png)
#### [execute()执行流程图](#execute-执行流程图)
![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/a61d8839c859-202506071547349.png)
#### [execute()源码](#execute-源码)
...
`newTaskFor`又调用`FutureTask`的有参构造器来创建一个`futureTask`实例，代码如下：
...
堆快照如下：
![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/6508c6ff11c9-202406112342211.webp)
如果是使用`ArrayBlockingQueue`的话，只有lambda实例这一个开销，所以只会使用320w个字节，约3.05MB内存，比起`LinkedBlockingQueue`少了一倍不止
![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/4dbaa3c0fa8b-202406112342798.webp)
#### [submit提交](#submit提交)
...
![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/619bbca644e5-202406112345411.webp)
如果使用的是`ArrayBlockingQueue`会省去Node的占用的内存。

#### [lambda中没有使用上下文中的其他变量](#lambda中没有使用上下文中的其他变量)

如果在lambda中没有使用到上下文的其他变量时，是不会重复创建lambda实例的，只会创建一个
![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/b3c74dee4e11-202406112348904.webp)
只会创建一个lambda实例
![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/70813614d014-202406112348956.webp)
如果配合上`ArrayBlockingQueue`以及execute，提交20w个任务的空间复杂度可以降至`O(1)`

因为20w个任务的实例都是同一个
![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/9f3d6038bb15-202406112348916.webp)
#### [小结](#小结)
...
#### [execute()提交](#execute-提交)

查看execute方法的执行逻辑
![java.util.concurrent.ThreadPoolExecutor#runWorker](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/0fb046668561-202406171749947.webp)java.util.concurrent.ThreadPoolExecutor#runWorker![java.util.concurrent.ThreadPoolExecutor#processWorkerExit](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/2ea29b9db7fe-202406171749972.webp)java.util.concurrent.ThreadPoolExecutor#processWorkerExit
可以发现，如果抛出异常，execute()提交的方式会移除抛出异常的线程，创建新的线程。

#### [submit()提交](#submit-提交)
![java.util.concurrent.AbstractExecutorService#submit(java.lang.Runnable)](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/5e5bbdafb111-202406171752757.webp)java.util.concurrent.AbstractExecutorService#submit(java.lang.Runnable)
可以发现，submit也是调用了execute方法，但是在调用之前,包装了一层 RunnableFuture，那一定是在RunnableFuture的实现 FutureTask中有特殊处理了，我们查看源码可以发现。
![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/dc40dac74523-202406171752066.webp)![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/0571de97c581-202406171752050.webp)![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/5060aaf4b1b0-202406171752072.webp)![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/e3c0bafb7976-202406171752076.webp)
也就是说，通过java.util.concurrent.FutureTask#get()，就可以获取对应的异常信息。

### [线程池是如何保活和回收的](#线程池是如何保活和回收的)

线程池的作用就是提高线程的利用率，需要线程时，可以直接从线程池中获取线程直接使用，而不用创建线程，那线程池中的线程，在没有任务执行时，是如何保活的呢?

在runWorker方法里，线程会循环getTask()获取阻塞队列中的任务。
![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/23730b02a29e-202404251113483.gif)
...
![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/9d4834bff063-202404251113491.gif)
...
![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/1f62ae643117-202404251113498.gif)
...
1. **使用 `awaitTermination()` 等待**​：设置合理超时时间（如 5-30 秒），等待任务自然完成

1. **超时后强制关闭**​：若超时仍未完成，调用 `shutdownNow()` 中断任务并返回未执行任务列表

1. **处理中断异常**​：在任务代码中响应 `InterruptedException`，确保中断信号能被正确处理
...
## [ExecutorService 线程池实例](#executorservice-线程池实例)

但是阿里巴巴开发手册中为什么不推荐使用Executors来创建线程池，这是为了让写的同学更加明确线程池的运行规则，规避资源耗尽的风险。
![](/imported/markdown/2026-02-27-markdown-ba3759d0-threadpoolexecutor详解-从核心参数到实现原理详解/images/2df2097e6b59-202410211134369.png)
