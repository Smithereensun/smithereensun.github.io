{
  ...
  "has_date": true,
  "description": "为什么要有 Buffer Pool？ 虽然说 MySQL 的数据是存储在磁盘里的，但是也不能每次都从磁盘里面读取数据，这样性能是极差的。 要想提升查询性能，那就加个缓存。所以，当数据从磁盘中取出后，缓存内存中，下次查询同样的数据的时候，直接从内存中读取。 为此，Innodb 存储引擎设计了一个缓冲池",
  "tags": [
    "数据库",
    "MySQL"
  ],
  "source": "local-markdown-library",
  "source_path": "database/mysql/01-basement3-linkedlists - MySQL性能背后的秘密武器：Buffer Pool与三大链表的奥秘.md",
  "date": "2025-08-09"
...
![](/imported/markdown/2025-08-09-markdown-2f19de8e-mysql性能背后的秘密武器-buffer-pool与三大链表的奥秘/images/c89b1709bf5b-202404261839227.png)

- 当读取数据时，如果数据存在于 Buffer Pool 中，客户端就会直接读取 Buffer Pool 中的数据，否则再去磁盘中读取。

- 当修改数据时，首先是修改 Buffer Pool 中数据所在的页，然后将其页设置为脏页，最后由后台线程将脏页写入到磁盘。

### [Buffer Pool里有什么](#buffer-pool里有什么)
...
![](/imported/markdown/2025-08-09-markdown-2f19de8e-mysql性能背后的秘密武器-buffer-pool与三大链表的奥秘/images/d982de87bcba-202404261839138.png)

Buffer Pool 除了缓存「索引页」和「数据页」，还包括了 undo 页，插入缓存、自适应哈希索引、锁信息等等。
![](/imported/markdown/2025-08-09-markdown-2f19de8e-mysql性能背后的秘密武器-buffer-pool与三大链表的奥秘/images/ccb27d0fd01b-202404261839103.png)

### [数据库启动的时候，是如何初始化Buffer Pool的](#数据库启动的时候-是如何初始化buffer-pool的)
...
![](/imported/markdown/2025-08-09-markdown-2f19de8e-mysql性能背后的秘密武器-buffer-pool与三大链表的奥秘/images/6c959e247b75-202404261839125.png)

只不过这个时候，Buffer Pool中的一个一个的**缓存页都是空的**，里面什么都没有，要等数据库运行起来之后，当对数据执行增删改查的操作的时候，才会把数据对应的页从磁盘文件里读取出来，放入Buffer Pool中的缓存页中。

## [管理Buffer Pool](#管理buffer-pool)

### [管理空闲页-free链表](#管理空闲页-free链表)

#### [如何知道哪些缓存页是空的](#如何知道哪些缓存页是空的)
...
![](/imported/markdown/2025-08-09-markdown-2f19de8e-mysql性能背后的秘密武器-buffer-pool与三大链表的奥秘/images/839909f5a7cf-202404261839177.png)
...
#### [磁盘上的页如何读取到Buffer Pool的缓存页中去？](#磁盘上的页如何读取到buffer-pool的缓存页中去)
...
#### [MySQL怎么知道某个数据页已经被缓存了](#mysql怎么知道某个数据页已经被缓存了)
...
![](/imported/markdown/2025-08-09-markdown-2f19de8e-mysql性能背后的秘密武器-buffer-pool与三大链表的奥秘/images/9663706eca80-202404261839238.png)
有了 Flush 链表后，后台线程就可以遍历 Flush 链表，将脏页写入到磁盘。

### [提高缓存命中率-LRU链表](#提高缓存命中率-lru链表)
...
![](/imported/markdown/2025-08-09-markdown-2f19de8e-mysql性能背后的秘密武器-buffer-pool与三大链表的奥秘/images/e67da3f0101f-202404261839094.png)

如果访问了 3 号的页，因为 3 号页在 Buffer Pool 里，所以把 3 号页移动到头部即可。
![](/imported/markdown/2025-08-09-markdown-2f19de8e-mysql性能背后的秘密武器-buffer-pool与三大链表的奥秘/images/616924e2aaf5-202404261839139.png)

而如果接下来，访问了 8 号页，因为 8 号页不在 Buffer Pool 里，所以需要先淘汰末尾的 5 号页，然后再将 8 号页加入到头部。
![](/imported/markdown/2025-08-09-markdown-2f19de8e-mysql性能背后的秘密武器-buffer-pool与三大链表的奥秘/images/c4a9215c268f-202404261839162.png)
...
![](/imported/markdown/2025-08-09-markdown-2f19de8e-mysql性能背后的秘密武器-buffer-pool与三大链表的奥秘/images/1bc5d6d2ed0a-202404261839108.png)

划分这两个区域后，**预读的页就只需要加入到 old 区域的头部**，当页被真正访问的时候，才将页插入 young 区域的头部。如果预读的页一直没有被访问，就会从 old 区域移除，这样**就不会影响 young 区域中的热点数据**。

#### [什么是 Buffer Pool 污染？](#什么是-buffer-pool-污染)
