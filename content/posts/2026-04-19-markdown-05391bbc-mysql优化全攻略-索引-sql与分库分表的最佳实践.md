{
  ...
  "has_date": true,
  "description": "SQL优化一般步骤 慢日志定位 通过慢查日志等定位那些执行效率较低的SQL语句 MySQL的慢查询日志是MySQL提供的一种日志记录，它用来记录在MySQL中响应时间超过阀值的语句，具体指运行时间超过long_query_time值的SQL，则会被记录到慢查询日志中。long_query_time的",
  "tags": [
    "数据库",
    "MySQL"
  ],
  "source": "local-markdown-library",
  "source_path": "database/mysql/04-tuning-overviewoftuning - MySQL优化全攻略：索引、SQL与分库分表的最佳实践.md",
  "date": "2026-04-19"
...
1. 过滤条件中的字段和order by 后跟的字段的顺序不一致，必然会出现 Using filesort，也就是说order by需要遵循最左匹配原则

1. order by后跟的排序字段是desc和asc 组合，不论排序顺序是否和组合索引顺序一致，必然会出现Using filesort

1. where条件的值确定(where xx = )，且order by后跟了where条件的排序字段(order by 字段去除定值字段后剩余单字段)，即使order by后跟的字段和组合索引字段顺序不一致，也不会出现 Using filesort
...
![](/imported/markdown/2026-04-19-markdown-05391bbc-mysql优化全攻略-索引-sql与分库分表的最佳实践/images/6350cca33c20-202404261921688.png)

第一行sql语句意思是，查询以id为500001开始的10条内容

执行第二条语句，执行计划是All，走的是全表扫描
![](/imported/markdown/2026-04-19-markdown-05391bbc-mysql优化全攻略-索引-sql与分库分表的最佳实践/images/97b39f116901-202404261921695.png)
...
![](/imported/markdown/2026-04-19-markdown-05391bbc-mysql优化全攻略-索引-sql与分库分表的最佳实践/images/f0a71f879797-202404261923116.png)
...
1. 接着，在应用程序中，需要使用分片算法将数据分散到不同的服务器上。常用的分片算法有hash分片、range分片和lookup分片。hash分片是将数据根据hash值分散到不同的服务器上；range分片是根据数据段的范围进行分片；lookup分片是通过路由表将数据指向相应的服务器。

1. 最后，需要使用MySQL集群中的代理节点来进行查询。代理节点收到查询请求后，会将请求分发到不同的服务器上并行查询，并将结果合并返回给应用程序。
...
### [count(主键字段)的执行过程](#count-主键字段-的执行过程)
...
### [count(*) 的执行过程(mysql官方文档推荐)](#count-的执行过程-mysql官方文档推荐)

count(*) 其实等于 count(0)，也就是说，当你使用 count(*) 时，MySQL 会将 * 参数转化为参数 0 来处理。

所以，count(*) 执行过程跟 count(1) 执行过程基本一样的，性能没有什么差异。

### [count(字段) 的执行过程](#count-字段-的执行过程)
...
1. **审计和历史记录**：逻辑删除保留了完整的数据历史记录，这对于审计和分析非常有用。

1. **数据完整性**：在某些情况下，逻辑删除可以更好地保持数据的完整性和一致性，尤其是在外键约束和关联数据存在的情况下。

1. **性能**：对于频繁删除和恢复操作的场景，逻辑删除可能比物理删除更高效，因为它避免了实际的删除操作和可能的索引重建。

1. **简化备份和恢复**：由于数据未被物理删除，备份和恢复过程可能更简单，因为不需要特殊处理已删除的数据。

缺点：

1. **数据膨胀**：逻辑删除会导致数据库中的数据量逐渐增加，因为被标记为删除的记录仍然占用存储空间。

1. **查询复杂性**：会导致数据库表垃圾数据越来越多，并且在查询时需要考虑逻辑删除的字段，从而影响查询效率

1. **索引维护**：逻辑删除的记录仍然存在于索引中，这可能会影响查询性能，尤其是在大型表中。

1. **事务管理**：在处理逻辑删除时，需要确保事务的一致性，这可能需要额外的工作和注意。

1. **安全性**：逻辑删除可能不足以保护敏感数据，因为数据仍然存在于数据库中，只是被标记为删除。
