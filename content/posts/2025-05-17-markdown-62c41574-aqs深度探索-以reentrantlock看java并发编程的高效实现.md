{
  ...
  "has_date": true,
  "description": "概述 AQS ( Abstract Queued Synchronizer ）是一个抽象的队列同步器，通过维护一个共享资源状态（ Volatile Int State ）来表示同步状态 和一个先进先出（ FIFO ）的线程**等待队列**来完成资源获取的排队工作，通过CAS完成对State值的修改。",
  "tags": [
    "Java",
    "并发"
  ],
  "source": "local-markdown-library",
  "source_path": "java/concurrent/03-juclock2-aqs - AQS深度探索：以ReentrantLock看Java并发编程的高效实现.md",
  "date": "2025-05-17"
...
AQS整体框架如下：
![](/imported/markdown/2025-05-17-markdown-62c41574-aqs深度探索-以reentrantlock看java并发编程的高效实现/images/094ae57755ef-202404251047981.gif)![](/imported/markdown/2025-05-17-markdown-62c41574-aqs深度探索-以reentrantlock看java并发编程的高效实现/images/1cd11f00e495-202404251047987.gif)
...
![](/imported/markdown/2025-05-17-markdown-62c41574-aqs深度探索-以reentrantlock看java并发编程的高效实现/images/24f59ca119ee-202404251047983.gif)
...
![](/imported/markdown/2025-05-17-markdown-62c41574-aqs深度探索-以reentrantlock看java并发编程的高效实现/images/ce6b738e1d77-202404251047977.gif)![](/imported/markdown/2025-05-17-markdown-62c41574-aqs深度探索-以reentrantlock看java并发编程的高效实现/images/ed0cb24c8bb4-202404251047997.gif)
### [CLH队列](#clh队列)

Craig、Landin and Hagersten队列，是单向链表，AQS中的队列是CLH变体的虚拟双向队列（FIFO），AQS是通过将每条请求共享资源的线程封装成一个节点来实现锁的分配。
![](/imported/markdown/2025-05-17-markdown-62c41574-aqs深度探索-以reentrantlock看java并发编程的高效实现/images/92e3804be927-202404251047000.gif)
AQS使用一个Volatile的int类型的成员变量来表示同步状态，通过内置的FIFO队列来完成资源获取的排队工作，通过CAS完成对State值的修改。

### [AQS的独占式和共享式](#aqs的独占式和共享式)
...
![](/imported/markdown/2025-05-17-markdown-62c41574-aqs深度探索-以reentrantlock看java并发编程的高效实现/images/16f69dd6e0dc-202404251047629.gif)
...
Node即为上面CLH变体队列中的节点。
![](/imported/markdown/2025-05-17-markdown-62c41574-aqs深度探索-以reentrantlock看java并发编程的高效实现/images/0b2eaacd1dd3-202404251047676.gif)
...
![](/imported/markdown/2025-05-17-markdown-62c41574-aqs深度探索-以reentrantlock看java并发编程的高效实现/images/41dcd4c31ca9-202404251047699.gif)
加锁：

1. 通过ReentrantLock的加锁方法Lock进行加锁操作。

1. 会调用到内部类 Sync的Lock方法，由于Sync#lock是抽象方法，根据 ReentrantLock初始化选择的公平锁和非公平锁，执行相关内部类的Lock方法，本质上都会执行AQS的 Acquire 方法。

1. AQS的 Acquire 方法会执行 tryAcquire 方法，但是由于tryAcquire需要自定义同步器实现，因此执行了ReentrantLock中的tryAcquire方法，由于ReentrantLock是通过公平锁和非公平锁内部类实现的tryAcquire方法，因此会根据锁类型不同，执行不同的tryAcquire。

1. tryAcquire是获取锁逻辑，获取失败后，会执行框架AQS的后续逻辑，跟ReentrantLock自定义同步器无关。

解锁：

1. 通过ReentrantLock的解锁方法Unlock进行解锁。

1. Unlock会调用内部类Sync的Release方法，该方法继承于AQS。

1. Release中会调用tryRelease方法，tryRelease需要自定义同步器实现，tryRelease只在ReentrantLock中的Sync实现，因此可以看出，释放锁的过程，并不区分是否为公平锁。

1. 释放成功后，所有处理由AQS框架完成，与自定义同步器无关。
...
1. acquireQueued()使线程阻塞在等待队列中获取资源，一直获取到资源后才返回。如果在整个等待过程中被中断过，则返回true，否则返回false。

1. 如果线程在等待过程中被中断过，它是不响应的。只是获取资源后才再进行自我中断selfInterrupt()，将中断补上。

![](/imported/markdown/2025-05-17-markdown-62c41574-aqs深度探索-以reentrantlock看java并发编程的高效实现/images/826a8f139b43-202404251047722.gif)
关于整个函数流程详解，可以往下看

#### [tryAcquire(int)](#tryacquire-int)
...
1. 将New中Node的Prev指针指向Pred。

1. 通过compareAndSetTail方法，完成尾节点的设置。这个方法主要是对tailOffset和Expect进行比较，如果tailOffset的Node和Expect的Node地址是相同的，那么设置Tail的值为Update的值。
...
![](/imported/markdown/2025-05-17-markdown-62c41574-aqs深度探索-以reentrantlock看java并发编程的高效实现/images/83f18c4a1454-202404251047743.gif)
...
![](/imported/markdown/2025-05-17-markdown-62c41574-aqs深度探索-以reentrantlock看java并发编程的高效实现/images/0c65badda540-202404251047762.gif)
...
![](/imported/markdown/2025-05-17-markdown-62c41574-aqs深度探索-以reentrantlock看java并发编程的高效实现/images/f620260fe130-202404251047278.gif)
...
#### [CANCELLED状态节点生成](#cancelled状态节点生成)
...
当前节点是尾节点：
![](/imported/markdown/2025-05-17-markdown-62c41574-aqs深度探索-以reentrantlock看java并发编程的高效实现/images/dc218dd99a8a-202404251047311.gif)
当前节点是Head的后继节点：
![](/imported/markdown/2025-05-17-markdown-62c41574-aqs深度探索-以reentrantlock看java并发编程的高效实现/images/8a1a048cf8a9-202404251047378.gif)
当前节点不是Head的后继节点，也不是尾节点：
![](/imported/markdown/2025-05-17-markdown-62c41574-aqs深度探索-以reentrantlock看java并发编程的高效实现/images/9c7911747f00-202404251047405.gif)
...
#### [tryRelease(int)](#tryrelease-int)

跟tryAcquire()一样，这个方法是需要独占模式的自定义同步器去实现的。正常来说，tryRelease()都会成功的，因为这是独占模式，该线程来释放资源，那么它肯定已经拿到独占资源了，直接减掉相应量的资源即可(state-=arg)，也不需要考虑线程安全的问题。但要注意它的返回值，上面已经提到了，release()是根据tryRelease()的返回值来判断该线程是否已经完成释放掉资源了！所以自义定同步器在实现时，如果已经彻底释放资源(state=0)，要返回true，否则返回false。

#### [unparkSuccessor(Node)](#unparksuccessor-node)
...
![](/imported/markdown/2025-05-17-markdown-62c41574-aqs深度探索-以reentrantlock看java并发编程的高效实现/images/e3e1b2bbaaaa-202404251047450.gif)

1. s.waitStatus &gt; 0，中间有节点取消时会出现（如超时）

![](/imported/markdown/2025-05-17-markdown-62c41574-aqs深度探索-以reentrantlock看java并发编程的高效实现/images/c58e56e8defb-202404251047479.gif)
...
### [acquireShared(int)](#acquireshared-int)
...
#### [doAcquireShared(int)](#doacquireshared-int)
...
## [ReentrantLock 的使用](#reentrantlock-的使用)
