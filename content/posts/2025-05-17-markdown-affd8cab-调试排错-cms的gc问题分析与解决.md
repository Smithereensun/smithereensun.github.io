{
  ...
  "has_date": true,
  "description": "来自美团技术团队。 写在前面 本文主要针对 Hotspot VM 中“CMS + ParNew”组合的一些使用场景进行总结。重点通过部分源码对根因进行分析以及对排查方法进行总结，排查过程会省略较多，另外本文专业术语较多，有一定的阅读门槛，如未介绍清楚，还请自行查阅相关材料。 引言 自 Sun 发布 ",
  "tags": [
    "Java",
    "JVM"
  ],
  "source": "local-markdown-library",
  "source_path": "java/jvm/03-debug5-cmsdebug - 调试排错 - CMS的GC问题分析与解决.md",
  "date": "2025-05-17"

}

来自[美团技术团队](https://tech.meituan.com/2020/11/12/java-9-cms-gc.html)。
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/d233418f6f3e-202404251921710.jpg)image.png
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/663fc85d4500-202404251921708.jpg)image.png
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/1e7088111c04-202404251921716.jpg)image.png
把 mark、sweep、compaction、copying 这几种动作的耗时放在一起看，大致有这样的关系：
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/94979c004715-202404251921718.jpg)image.png
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/9ca1b4eb59a9-202404251921721.jpg)image.png
备注：值得一提的是，早些年国内 GC 技术的布道者 RednaxelaFX （江湖人称 R 大）也曾就职于 Azul，本文的一部分材料也参考了他的一些文章。

#### [2.5.1 分代收集器](#_2-5-1-分代收集器)

l ParNew： 一款多线程的收集器，采用复制算法，主要工作在 Young 区，可以通过 -XX:ParallelGCThreads 参数来控制收集的线程数，整个过程都是 STW 的，常与 CMS 组合使用。

l CMS： 以获取最短回收停顿时间为目标，采用“标记-清除”算法，分 4 大步进行垃圾收集，其中初始标记和重新标记会 STW，多数应用于互联网站或者 B/S 系统的服务器端上，JDK9 被标记弃用，JDK14 被删除，详情可见 [JEP 363](https://openjdk.java.net/jeps/363)。

#### [2.5.2 分区收集器](#_2-5-2-分区收集器)

l G1： 一种服务器端的垃圾收集器，应用在多处理器和大容量内存环境中，在实现高吞吐量的同时，尽可能地满足垃圾收集暂停时间的要求。

l ZGC： JDK11 中推出的一款低延迟垃圾回收器，适用于大内存低延迟服务的内存管理和回收，SPECjbb 2015 基准测试，在 128G 的大堆下，最大停顿时间才 1.68 ms，停顿时间远胜于 G1 和 CMS。

l Shenandoah： 由 Red Hat 的一个团队负责开发，与 G1 类似，基于 Region 设计的垃圾收集器，但不需要 Remember Set 或者 Card Table 来记录跨 Region 引用，停顿时间和堆的大小没有任何关系。停顿时间与 ZGC 接近，下图为与 CMS 和 G1 等收集器的 benchmark。
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/89fe4a26f413-202404251921722.jpg)image.png
#### [2.5.3 常用收集器](#_2-5-3-常用收集器)

目前使用最多的是 CMS 和 G1 收集器，二者都有分代的概念，主要内存结构如下：
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/3956de35cb2a-202404251921499.jpg)image.png
#### [2.5.4 其他收集器](#_2-5-4-其他收集器)
...
#### [2.6.1 命令行终端](#_2-6-1-命令行终端)

- 标准终端类：jps、jinfo、jstat、jstack、jmap

- 功能整合类：jcmd、vjtools、arthas、greys

#### [2.6.2 可视化界面](#_2-6-2-可视化界面)
...
## [3. GC 问题判断](#_3-gc-问题判断)

在做 GC 问题排查和优化之前，我们需要先来明确下到底是不是 GC 直接导致的问题，或者应用代码导致的 GC 异常，最终出现问题。

### [3.1 判断 GC 有没有问题？](#_3-1-判断-gc-有没有问题)

#### [3.1.1 设定评价标准](#_3-1-1-设定评价标准)
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/147963005b0b-202404251921517.jpg)image.png
...
#### [3.1.2 读懂 GC Cause](#_3-1-2-读懂-gc-cause)

拿到 GC 日志，我们就可以简单分析 GC 情况了，通过一些工具，我们可以比较直观地看到 Cause 的分布情况，如下图就是使用 gceasy 绘制的图表：
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/73d49f56549f-202404251921539.jpg)image.png
...
### [3.2 判断是不是 GC 引发的问题？](#_3-2-判断是不是-gc-引发的问题)
...
### [3.3 问题分类导读](#_3-3-问题分类导读)

#### [3.3.1 Mutator 类型](#_3-3-1-mutator-类型)
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/10be50cacc65-202404251921559.jpg)image.png
#### [3.3.2 GC 问题分类](#_3-3-2-gc-问题分类)
...
## [4. 常见场景分析与解决](#_4-常见场景分析与解决)

### [4.1 场景一：动态扩容引起的空间震荡](#_4-1-场景一-动态扩容引起的空间震荡)
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/ae402a91bc8c-202404251921579.jpg)image.png
#### [4.1.2 原因](#_4-1-2-原因)
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/ed31bca611f6-202404251921603.jpg)image.png
#### [4.1.3 策略](#_4-1-3-策略)
...
### [4.2 场景二：显式 GC 的去与留](#_4-2-场景二-显式-gc-的去与留)
...
找到 System.gc 在 Hotspot 中的源码，可以发现增加 -XX:+DisableExplicitGC 参数后，这个方法变成了一个空方法，如果没有加的话便会调用 Universecollect 方法，继续跟进到这个方法中，发现 System.gc 会引发一次 STW 的 Full GC，对整个堆做收集。
...
-
**去掉 System.gc：**如果禁用掉的话就会带来另外一个内存泄漏问题，此时就需要说一下 DirectByteBuffer，它有着零拷贝等特点，被 Netty 等各种 NIO 框架使用，会使用到堆外内存。堆内存由 JVM 自己管理，堆外内存必须要手动释放，DirectByteBuffer 没有 Finalizer，它的 Native Memory 的清理工作是通过 sun.misc.Cleaner 自动完成的，是一种基于 PhantomReference 的清理工具，比普通的 Finalizer 轻量些。
...
### [4.3 场景三：MetaSpace 区 OOM](#_4-3-场景三-metaspace-区-oom)
...
- Klass MetaSpace： 就是用来存 Klass 的，存放类的元数据，它包含了类的定义信息，比如类的名称、访问修饰符、常量池、字段描述、方法描述等。这部分元数据直接关联到类加载器，因此，每个类加载器都有自己的Klass Metaspace。这部分默认放在 Compressed Class Pointer Space 中，是一块连续的内存区域，紧接着 Heap。Compressed Class Pointer Space 不是必须有的，如果设置了 -XX:-UseCompressedClassPointers，或者 -Xmx 设置大于 32 G，就不会有这块内存，这种情况下 Klass 都会存在 NoKlass Metaspace 里。

- NoKlass MetaSpace： 专门来存 Klass 相关的其他的内容，比如 类元数据之外的其他元数据，如 Method，ConstantPool，方法的字节码、内部字符串以及其他JVM内部使用的结构。可以由多块不连续的内存组成。虽然叫做 NoKlass Metaspace，但是也其实可以存 Klass 的内容，上面已经提到了对应场景。 具体的定义都可以在源码 shared/vm/memory/metaspace.hpp 中找到：
...
- MetaSpace 弹性伸缩： 由于 MetaSpace 空间和 Heap 并不在一起，所以这块的空间可以不用设置或者单独设置，一般情况下避免 MetaSpace 耗尽 VM 内存都会设置一个 MaxMetaSpaceSize，在运行过程中，如果实际大小 小于这个值，JVM 就会通过 -XX:MinMetaspaceFreeRatio 和 -XX:MaxMetaspaceFreeRatio 两个参数动态控制整个 MetaSpace 的大小，具体使用可以看 MetaSpaceGCinc_capacity_until_GC() 和 MetaspaceGC::dec_capacity_until_GC() 进行 expand 或 shrink，这个过程也可以参照场景一中的伸缩模型进行理解。
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/eae6ae9278c8-202404251921448.jpg)image.png
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/df6558e4cc94-202404251921472.jpg)image.png
过早晋升的危害：
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/b3b434c155dc-202404251921494.jpg)image.png
如果是分配速率过大：
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/0a2e5fc5105f-202404251921514.jpg)image.png![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/1006ea1647f0-202404251921538.jpg)image.png
...
### [4.5 场景五：CMS Old GC 频繁*](#_4-5-场景五-cms-old-gc-频繁)
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/6739a3e4daf1-202404251921560.jpg)image.png
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/192ebb7d1293-202404251921158.jpg)image.png
#### [4.5.4 小结](#_4-5-4-小结)

经过整个流程下来基本就能定位问题了，不过在优化的过程中记得使用**控制变量**的方法来优化，防止一些会加剧问题的改动被掩盖。

### [4.6 场景六：单次 CMS Old GC 耗时长*](#_4-6-场景六-单次-cms-old-gc-耗时长)

#### [4.6.1 现象](#_4-6-1-现象)

CMS GC 单次 STW 最大超过 1000ms，不会频繁发生，如下图所示最长达到了 8000ms。某些场景下会引起“雪崩效应”，这种场景非常危险，我们应该尽量避免出现。
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/5b82fbc0f5bb-202404251921180.jpg)image.png
#### [4.6.2 原因](#_4-6-2-原因)
...
核心代码都在 /src/hotspot/share/gc/cms/concurrentMarkSweepGeneration.cpp 中，内部有个线程 ConcurrentMarkSweepThread 轮询来校验，Old 区的垃圾回收相关细节被完全封装在 CMSCollector 中，调用入口就是 ConcurrentMarkSweepThread 调用的 CMSCollectorcollect 方法，此处我们讨论大多数场景的 collect_in_background。整个过程中会 STW 的主要是 initial Mark 和 Final Remark，核心代码在 VM_CMS_Initial_Mark / VM_CMS_Final_Remark 中，执行时需要将执行权交由 VMThread 来执行。

- CMS Init Mark执行步骤，实现在 CMSCollectorwork 中，整体步骤和代码如下：

![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/ef13df533078-202404251921198.jpg)image.png
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/66fac389dee6-202404251921221.jpg)image.png
...
### [4.7 场景七：内存碎片&收集器退化](#_4-7-场景七-内存碎片-收集器退化)
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/68264f18d68a-202404251921248.jpg)image.png
碎片带来了两个问题：
...
### [4.8 场景八：堆外内存 OOM](#_4-8-场景八-堆外内存-oom)
...
1. 原因二：通过 JNI 调用的 Native Code 申请的内存未释放

这种情况排查起来比较困难，我们可以通过 Google perftools + Btrace 等工具，帮助我们分析出问题的代码在哪里。

gperftools 是 Google 开发的一款非常实用的工具集，它的原理是在 Java 应用程序运行时，当调用 malloc 时换用它的 [libtcmalloc.so](http://libtcmalloc.so)，这样就能对内存分配情况做一些统计。我们使用 gperftools 来追踪分配内存的命令。如下图所示，通过 gperftools 发现 Java_java_util_zip_Inflater_init 比较可疑。
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/f7d8aa07c432-202404251921271.jpg)image.png
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/5ca29c0cdd7a-202404251921924.jpg)image.png
最终定位到是，项目中对 GIPInputStream 的使用错误，没有正确的 close()。
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/50272fd9b199-202404251921948.jpg)image.png
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/10a89b488090-202404251921974.jpg)image.png
### [4.9 场景九：JNI 引发的 GC 问题](#_4-9-场景九-jni-引发的-gc-问题)
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/a5b7e6ece24a-202404251921994.jpg)image.png
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/d45dfa821622-202404251921020.jpg)image.png
#### [4.9.4 小结](#_4-9-4-小结)
...
### [5.1 处理流程（SOP）](#_5-1-处理流程-sop)

下图为整体 GC 问题普适的处理流程，重点的地方下面会单独标注，其他的基本都是标准处理流程，此处不再赘述，最后在整个问题都处理完之后有条件的话建议做一下复盘。
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/213abb47b934-202404251921044.jpg)image.png
...
### [5.2 根因鱼骨图](#_5-2-根因鱼骨图)

送上一张问题根因鱼骨图，一般情况下我们在处理一个 GC 问题时，只要能定位到问题的“病灶”，有的放矢，其实就相当于解决了 80%，如果在某些场景下不太好定位，大家可以借助这种根因分析图通过排除法去定位。
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/608752d5b7b6-202404251921864.jpg)image.png
### [5.3 调优建议](#_5-3-调优建议)
...
![image.png](/imported/markdown/2025-05-17-markdown-affd8cab-调试排错-cms的gc问题分析与解决/images/f8cf0fd67c68-202404251921888.jpg)image.png
