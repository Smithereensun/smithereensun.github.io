{
  ...
  "title": "内存管理 - 内存页面置换算法",
  "has_date": true,
  "description": "概述 当 CPU 访问的页面不在物理内存时，便会产生一个缺页中断，请求操作系统将所缺页调入到物理内存。但内存已无空闲空间时，这时候，就需要**页面置换算法**选择一个物理页，如果该物理页有被修改过（脏页），则把它换出到磁盘，然后把该被置换出去的页表项的状态改成「无效的」，最后把正在访问的页面装入到这",
  "tags": [
    "计算机基础"
  ],
  "source": "local-markdown-library",
  "source_path": "cs-basics/operating-system/memorypagereplacementalgorithm - 内存管理 - 内存页面置换算法.md",
  "date": "2025-05-17"
...
这里先引出页表项中的字段
![](/imported/markdown/2025-05-17-markdown-d3eb9aaa-内存管理-内存页面置换算法/images/8290bc651da2-202404271130724.png)
...
![](/imported/markdown/2025-05-17-markdown-d3eb9aaa-内存管理-内存页面置换算法/images/bc77d4157930-202404271130369.png)

这很理想，但是实际系统中无法实现，因为操作系统无法得知每个页面要等待多长时间以后才会再次被访问，所以这种算法无法在现实中实现，通过用作页面置换算法的一种评价标准

## [最近最久未使用的置换算法LRU](#最近最久未使用的置换算法lru)

虽然无法知道将来要使用的页面情况，但是可以知道过去使用页面的情况。

最近最久未使用（LRU）的置换算法的基本思路是，发生缺页时，选择最长时间没有被访问的页面进行置换，也就是说，该算法假设已经很久没有使用的页面很有可能在未来较长的一段时间内仍然不会被使用。
![](/imported/markdown/2025-05-17-markdown-d3eb9aaa-内存管理-内存页面置换算法/images/b8b3e96da841-202404271130302.png)
...
![](/imported/markdown/2025-05-17-markdown-d3eb9aaa-内存管理-内存页面置换算法/images/ec995a7ec645-202404271130193.png)
...
![](/imported/markdown/2025-05-17-markdown-d3eb9aaa-内存管理-内存页面置换算法/images/b88a914fe99e-202404271130516.png)
