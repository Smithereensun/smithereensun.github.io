{
  ...
  "has_date": true,
  "description": "今天来讲讲 Redis 的请求监听，通俗点说，就是Redis是如何监听客户端发出的set、get等命令的。 基础架构 众所周知，Redis 是单进程单线程架构，虽然是单进程单线程，但是Redis的性能却毫不逊色，能轻松应对一般的高并发场景，那么Redis究竟是施了什么魔法呢？ 其实 Redis 的原",
  "tags": [
    "数据库",
    "Redis"
  ],
  "source": "local-markdown-library",
  "source_path": "database/redis/02-basement4-handlecommands - Redis命令处理机制源码探究.md",
  "date": "2025-05-17"
...
其实 Redis 的原理和 Nginx 差不多，都利用了 **IO 多路复用**来提高处理能力，所谓多路复用，就是一个线程可以同时处理多个IO操作，当 某个 IO 操作 Ready 时，操作系统会主动通知进程。使用 IO 多路复用，我们可以使用 epoll、kqueue、select，API 都差不多。
...
### [启动事件监听循环，监听请求](#启动事件监听循环-监听请求)

经过了上面的一系列初始化操作，Redis 就会正式进入事件监听的循环中，即 aeMain（）。

这段代码看起来很简单，只要事件监听没有停止，Redis 就会一直循环调用aeProcests（）处理事件。

aeProcests（）中的核心逻辑就是两步：

#### [调用 aeApiPoll （）来等待接收事件。](#调用-aeapipoll-来等待接收事件。)

我们看下使用kqueue时，ae_kqueue.c 中的实现。

这里调用的是 kevent（）来接收事件。kevent（）描述如下：

#### [遍历返回的就绪事件数组，处理事件。](#遍历返回的就绪事件数组-处理事件。)
...
1. 创建内核事件队列，并注册上述的套接字描述符到队列中。

1. 开启循环，监听队列中的就绪事件。

1. 当端口有新事件时，调用 accept（）与新客户端建立连接，并再次将新连接的描述符注册到内核事件队列中，监听该TCP连接上的事件。

1. 当与客户端的TCP连接上有新事件时，调用对应的事件处理函数，从该连接上读取并解析请求，并执行请求对应的命令。
