{
  ...
  "has_date": true,
  "description": "概述 Redis的高可用机制有持久化、复制、哨兵和集群。其主要的作用和解决的问题分别是： 持久化：持久化是最简单的高可用方法(有时甚至不被归为高可用的手段)，**主要作用是数据备份**，即将数据存储在硬盘，保证数据不会因进程退出而丢失。 复制：复制是高可用Redis的基础，哨兵和集群都是在复制基础上",
  "tags": [
    "数据库",
    "Redis"
  ],
  "source": "local-markdown-library",
  "source_path": "database/redis/04-ha2-sentinelmechanism - Redis哨兵机制是如何优化主从切换流程的？.md",
  "date": "2025-05-17"
...
### [监控：如何判断主库是否真的挂了](#监控-如何判断主库是否真的挂了)
...
![](/imported/markdown/2025-05-17-markdown-9d0d74eb-redis哨兵机制是如何优化主从切换流程的/images/866545be68e5-202404270807815.png)
![](/imported/markdown/2025-05-17-markdown-9d0d74eb-redis哨兵机制是如何优化主从切换流程的/images/70a9d4f170bb-202404270807347.png)

当这个哨兵的赞同票数达到哨兵配置文件中的 `quorum` 配置项设定的值后，这时主节点就会被该哨兵标记为客观下线。

"客观下线"：N个哨兵实例，最好要有N/2+1个实例判断主库为"主观下线"，才能判断为"客观下线"。

#### [由哪个哨兵进行主从故障转移](#由哪个哨兵进行主从故障转移)
...
选定新主库是 筛选 ➕ 打分的过程
![](/imported/markdown/2025-05-17-markdown-9d0d74eb-redis哨兵机制是如何优化主从切换流程的/images/bc02a2229b6d-202404270807831.png)
#### [筛选条件：](#筛选条件)
...
![](/imported/markdown/2025-05-17-markdown-9d0d74eb-redis哨兵机制是如何优化主从切换流程的/images/3f3c4e5283bb-202404270807275.png)
### [通知](#通知)

#### [将选举出的从节点解除从节点身份，升级为主节点](#将选举出的从节点解除从节点身份-升级为主节点)

在选举出从节点为新主节点后后，哨兵 leader 向被选中的从节点发送 SLAVEOF no one 命令，让这个从节点解除从节点的身份，将其变为新主节点。
![](/imported/markdown/2025-05-17-markdown-9d0d74eb-redis哨兵机制是如何优化主从切换流程的/images/9c11449264c7-202404270808071.png)

#### [将从节点指向新的主节点](#将从节点指向新的主节点)

哨兵 leader 下一步要做的就是，让已下线主节点属下的所有从节点指向新主节点，这一动作可以通过向从节点发送 SLAVEOF 命令来实现。
![](/imported/markdown/2025-05-17-markdown-9d0d74eb-redis哨兵机制是如何优化主从切换流程的/images/926347cf8c6e-202404270808415.png)

#### [通知客户端主节点已经更换](#通知客户端主节点已经更换)

这主要通过 Redis 的发布者/订阅者机制来实现的。每个哨兵节点提供发布者/订阅者机制，客户端可以从哨兵订阅消息。

哨兵提供的消息订阅频道有很多，不同频道包含了主从节点切换过程中的不同关键事件，几个常见的事件如下：
![](/imported/markdown/2025-05-17-markdown-9d0d74eb-redis哨兵机制是如何优化主从切换流程的/images/507326b7ac57-202404270808694.jpeg)

客户端和哨兵建立连接后，客户端会订阅哨兵提供的频道。主从切换完成后，**哨兵就会向 +switch-master 频道发布新主节点的 IP 地址和端口的消息，这个时候客户端就可以收到这条信息，然后用这里面的新主节点的 IP 地址和端口进行通信了**。

#### [将原主节点变为从节点，指向新的主节点](#将原主节点变为从节点-指向新的主节点)

继续监视旧主节点，当旧主节点重新上线时，哨兵集群就会向它发送 SLAVEOF 命令，让它成为新主节点的从节点
![](/imported/markdown/2025-05-17-markdown-9d0d74eb-redis哨兵机制是如何优化主从切换流程的/images/2cdb06c54da4-202404270808863.png)
...
1. 第二轮投票：选出哨兵leader，决定由哪个哨兵执行主从切换

1. 由哨兵 leader 进行选主

1. 由哨兵 leader 进行通知，实现主从故障转移
