{
  ...
  "has_date": true,
  "description": "Mysql命令大全 mysql服务的启动和停止 连接mysql 注意，如果是连接到另外的机器上，则需要加入一个参数-h机器IP 键入命令mysql -u root -p， 回车后提示输入密码，然后回车即可进入到mysql中了 权限控制-DCL 权限验证过程 mysql中存在4个控制权限的表， mys",
  "tags": [
    "数据库",
    "MySQL"
  ],
  "source": "local-markdown-library",
  "source_path": "database/mysql/mysql-use - MySQL使用全攻略：从命令大全到规范的最佳实践.md",
  "date": "2026-04-19"
...
1. 通过身份认证后，进行权限分配，按照user，db，tables_priv，columns_priv的顺序进行验证。即

  1. 先检查全局权限表user，如果user中对应的权限为Y，则此用户对所有数据库的权限都为Y，将不再检查db, tables_priv,columns_priv；

  1. 如果user中为N，则到db表中检查此用户对应的具体数据库，并得到db中为Y的权限；（一般来说，权限管理只会到db层，过于细化也不方便管理）

  1. 如果db中为N，则检查table_priv和columns_priv(如果是存储过程操作则检查mysql.procs_priv)，如果满足，则执行操作

  1. 如果以上检查均失败，则系统拒绝执行操作。

因为MySQL是使用User和Host两个字段来确定用户身份的，这样就带来一个问题，就是一个客户端到底属于哪个host。如果一个客户端同时匹配几个Host，对用户的确定将按照下面的优先级来排：`基本观点`越精确的匹配越优先

- Host列上，越是确定的Host越优先，[localhost, 192.168.1.1, wiki..top] 优先于[192.168.%, %..top]，优先于[192.%, %.top]，优先于[%]
...
1. 当使用 `ON DUPLICATE KEY UPDATE` 插入数据时，如果插入的数据导致唯一索引或主键冲突，SQL 引擎不会报错或忽略该操作，而是会执行指定的更新操作。

假设有一个表 `users`，该表的主键是 `id`，且表中已经有id为1的数据，则以下语句会把name更新为2
...
#### [将文本数据导入数据库](#将文本数据导入数据库)

文本数据的字段数据之间用tab键隔开。

### [数据操作-select查询语句](#数据操作-select查询语句)
...
1. select 查询字段 from 表列表名/视图列表名 where 条件 group by (列列表) having 条件
 执行顺序：先 from 再 where 再 group by 再 having 最后select

1. select 查询字段 from 表列表名/视图列表名 where 条件 group by (列列表) having 条件 order by 列列表
 执行顺序：先 from 再 where 再 group by 再 having 再 select 最后 order by

1. select 查询字段 from 表1 join 表2 on 表1.列1=表2.列1...join 表n on 表n.列1=表(n-1).列1 where 表1.条件 and 表2.条件...表n.
...
1. **应用连接条件**：然后，`INNER JOIN` 会根据指定的连接条件（通常是相等条件）过滤这些组合，只保留那些在连接条件上匹配的行。

1. **生成结果集**：最后，根据过滤后的匹配行生成最终结果集。
...
​ ![](/imported/markdown/2026-04-19-markdown-dbb7773b-mysql使用全攻略-从命令大全到规范的最佳实践/images/3ee077760bfd-202407291702266.png)
...
![](/imported/markdown/2026-04-19-markdown-dbb7773b-mysql使用全攻略-从命令大全到规范的最佳实践/images/d3a328787fc7-202407291702188.png)
## [相关规范](#相关规范)

### [数据库范式规范](#数据库范式规范)

数据库的范式（Normal Form）是设计关系型数据库时遵循的一系列规则和规范。其核心目的是**减少数据冗余**，并避免在增删改数据时出现异常，从而保证数据的一致性和完整性。

在实际应用中，我们通常关注前三个范式（1NF, 2NF, 3NF），它们已经能解决绝大部分的设计问题。

#### [范式速览](#范式速览)
范式核心目标关键动作**1NF**保证数据原子性拆分复合列，确保一格一值**2NF**消除部分依赖拆分只依赖部分复合主键的列**3NF**消除传递依赖拆分依赖其他非主键列的列
#### [第一范式 (1NF)：保证“原子性”](#第一范式-1nf-保证-原子性)

**核心要求**：表中的每一列都必须是不可再分的原子值，确保“一格一值”。

- **违规示例**：一个“联系方式”列里同时存储了“电话”和“邮箱”，或者一个“选修课程”列里用逗号分隔存储了“数学, 英语”。

- **如何修正**：将复合值拆分成独立的列。例如，将“联系方式”拆分为“电话”和“邮箱”两列；或者为“选修课程”创建一个独立的学生-课程关联表。

#### [第二范式 (2NF)：消除“部分依赖”](#第二范式-2nf-消除-部分依赖)

**核心要求**：在满足1NF的基础上，表中的非主键列必须**完全依赖**于整个主键，而不能只依赖主键的一部分。

- **前提**：此规则主要针对拥有**复合主键**（由两个或多个字段共同组成主键）的表。

- **违规示例**：一张“订单详情表”，主键是 `(订单ID, 商品ID)`。如果表中包含“下单日期”列，那么它就违反了2NF，因为“下单日期”只依赖于“订单ID”，而与“商品ID”无关。

- **如何修正**：将只依赖于部分主键的字段拆分出去。上例中，应将“下单日期”移到以“订单ID”为主键的“订单表”中。

#### [第三范式 (3NF)：消除“传递依赖”](#第三范式-3nf-消除-传递依赖)

**核心要求**：在满足2NF的基础上，表中的非主键列必须**直接依赖**于主键，而不能依赖于其他非主键列。

- **通俗理解**：表里的非主键字段，除了主键，谁也别靠。

- **违规示例**：一张“员工表”，主键是“员工ID”，但表中同时包含“部门ID”和“部门名称”。这里“部门名称”依赖于“部门ID”，而“部门ID”又依赖于“员工ID”，形成了传递依赖。

- **如何修正**：将存在传递依赖的字段拆分出去。上例中，应创建一个独立的“部门表”（包含部门ID和部门名称），员工表中只保留“部门ID”作为外键。

#### [实践中的平衡](#实践中的平衡)

遵循范式能让数据库结构更清晰、数据更一致。但在实际项目中，并非范式越高越好。过高的范式意味着更多的表拆分，查询时需要进行大量的表连接（JOIN）操作，可能会影响性能。

因此，数据库设计往往是在**数据一致性**和**查询性能**之间寻找平衡。常见的做法是先按第三范式（3NF）进行设计，然后根据实际的性能需求，有策略地进行“反范式化”（Denormalization），即适当增加一些冗余字段来减少连接查询，提升读取效率。
...
### [规范中提到的点的解释](#规范中提到的点的解释)

#### [为什么建议用自增id作为主键](#为什么建议用自增id作为主键)
...
1. 如果使用非自增主键，由于每次插入主键的索引值都是随机的，因此每次插入新的数据时，就可能会插入到现有数据页中间的某个位置，那么就不得不移动其它数据来满足新数据的插入，当前页插入不下了，那就会发生页分裂。造成额外的开销。页分裂还有可能会造成大量的内存碎片，导致索引结构不紧凑，从而影响查询效率。

InnoDB 存储引擎会根据不同的场景选择不同的列作为索引：

1. 如果有主键，默认会使用主键作为聚簇索引的索引键（key）；

1. 如果没有主键，就选择第一个不包含 NULL 值的唯一列作为聚簇索引的索引键（key）；

1. 在上面两个都没有的情况下，InnoDB 将自动生成一个隐式自增 id 列作为聚簇索引的索引键（key）；
...
![](/imported/markdown/2026-04-19-markdown-dbb7773b-mysql使用全攻略-从命令大全到规范的最佳实践/images/f2c0efdd5704-202407310956190.png)
