{
  ...
  "has_date": true,
  "description": "事务的四个特性ACID 原子性（Atomicity）：语句要么全执行，要么全不执行，是事务最核心的特性，事务本身就是以原子性来定义的；实现主要基于**undo log** 持久性（Durability）：保证事务提交后不会因为宕机等原因导致数据丢失；实现主要基于**redo log** 隔离性（Is",
  "tags": [
    "数据库",
    "MySQL"
  ],
  "source": "local-markdown-library",
  "source_path": "database/mysql/01-basement5-transactions-isolationlevel - MySQL事务原理：从ACID到隔离级别的全解析.md",
  "date": "2025-08-09"
...
## [并发事务中可能存在的问题](#并发事务中可能存在的问题)

### [脏读](#脏读)

当前事务(A)中可以**读到其他事务(B)未提交的数据**（脏数据），这种现象是脏读。举例如下（以账户余额表为例）：
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/24e9ccbd8d96-202404261856440.png)

### [不可重复读](#不可重复读)

在事务A中先后两次读取同一个数据，两次读取的结果不一样，这种现象称为不可重复读，**读取到了其它事务已提交的数据**。脏读与不可重复读的区别在于：前者读到的是其他事务未提交的数据，后者读到的是其他事务已提交的数据。举例如下：
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/cdea46bd367e-202404261856404.png)
### [幻读](#幻读)

在事务A中按照某个条件先后两次查询数据库，两次查询结果的条数不同，这种现象称为幻读。不可重复读与幻读的区别可以通俗的理解为：前者是数据变了，后者是数据的行数变了。举例如下：
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/4eab99eef10c-202404261856474.png)
### [总结](#总结)
...
三者严重性排序：
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/528837f272c6-202404261856467.png)

## [事务的隔离级别 - 即隔离性](#事务的隔离级别-即隔离性)
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/9807d5adca84-202404261856480.png)
...
### [四种隔离级别是如何实现的](#四种隔离级别是如何实现的)

1. 对于**读未提交**隔离级别的事务来说，因为可以读到未提交事务修改的数据，所以直接读取最新的数据就好了；

1. 对于**串行化**隔离级别的事务来说，通过加读写锁的方式来避免并行访问；

1. 对于**读已提交**和**可重复读**隔离级别的事务来说，它们是通过 Read View 来实现的，它们的区别在于创建 Read View 的时机不同， Read View 就相当于一个数据快照。**读已提交**隔离级别是在**每个语句执行前**重新生成一个 **Read View**，而**可重复读**隔离级别是**启动事务时**生成一个 Read View，然后整个事务期间都在用这个 Read View。

### [InnoDB可重复读可以尽量避免幻读](#innodb可重复读可以尽量避免幻读)
...
1. 针对当前读（select ... for update 等语句），是通过 next-key lock（记录锁+间隙锁）（详情请看Mysql的锁）方式解决了幻读，因为当执行 select ... for update 语句的时候，会加上 next-key lock，如果有其他事务在 next-key lock 锁范围内插入了一条记录，那么这个**插入语句就会被阻塞**，无法成功插入，所以就很好了避免幻读问题。
...
### [MVCC快照读没有完全解决幻读](#mvcc快照读没有完全解决幻读)
...
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/af3ac39ecfaf-202404261856465.png)
...
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/4a36299229c8-202404261856401.png)

两个事务的操作如下：
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/776bfbb1f048-202404261856391.png)

- t2时刻，因为查询不到这一行数据，因此没有输出。

- t3时刻，事务B插入了一条数据，因此，插入的id为5的那一行数据的trx_id的值为事物B的id，也就是52。
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/8e04da632b4f-202404261856405.png)

- t4时刻，事物A对于这一行数据进行修改。因为修改操作，不受trx_id的限制。因此，在t4时刻，事物A会把这一行数据的trx_id改变为51.
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/8691b1a09c14-202404261856388.png)
...
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/b2ff871bc445-202404261856418.png)
...
### [Read View 的四个重要字段](#read-view-的四个重要字段)
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/2bf2c4c98206-202404261856585.png)

1. creator_trx_id ：指的是创建该 Read View 的事务的事务 id。

1. m_ids ：指的是在创建 Read View 时，当前数据库中 **活跃事务** 的事务 id 列表，注意这是一个列表， **活跃事务** 指的就是，启动了但还没提交的事务。
...
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/1af24c995737-202404261856176.png)

### [undolog中每条记录的两个隐藏列](#undolog中每条记录的两个隐藏列)
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/0679bc855ef4-202404261856242.png)

- trx_id，当一个事务对某条聚簇索引记录进行改动时，就会把该事务的事务 id 记录在 trx_id 隐藏列里；

- roll_pointer，每次对某条聚簇索引记录进行改动时，都会把旧版本的记录写入到 undo 日志中，然后这个隐藏列是个指针，指向每一个旧版本记录，于是就可以通过它找到修改前的记录。

### [Read View的规则](#read-view的规则)
...
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/5ce90a14c74d-202404261856156.png)

假设事务 A （事务 id 为51）启动后，紧接着事务 B （事务 id 为52）也启动了，那这两个事务创建的 Read View 如下：
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/aaa4891bbe98-202404261856185.png)
...
1. 事务 B 读取zhangsan的账户余额记录，读到余额是 100 ；（事务B第一次读）

1. 事务 A 将zhangsan的账户余额记录修改成 200 万，并没有提交事务；

1. 事务 B 读取zhangsan的账户余额记录，读到余额还是 100 ；（事务B第二次读）

1. 事务 A 提交事务；

1. 事务 B 读取zhangsan的账户余额记录，读到余额依然还是 100 ；（事务B第三次读）
...
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/9b912021d066-202404261856223.png)
...
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/9c1c7324426a-202404261856664.png)

假设事务 A （事务 id 为51）启动后，紧接着事务 B （事务 id 为52）也启动了，接着按顺序执行了以下操作：

1. 事务 A 读取数据（创建 Read View），zhangsan的账户余额为 100 ；

1. 事务 B 读取数据（创建 Read View），zhangsan的账户余额为 100 ；（事务B第一次读）

1. 事务 A 修改数据（还没提交事务），将zhangsan的账户余额从 100 修改成了 200 ；

1. 事务 B 读取数据（创建 Read View），zhangsan的账户余额为 100 ；（事务B第二次读）

1. 事务 A 提交事务；

1. 事务 B 读取数据（创建 Read View），zhangsan的账户余额为 200 ；（事务B第三次读）

**事务B第一次读**时创建的Read View：
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/2c62fa4560bb-202404261856925.png)
...
接着事务A修改数据（还没提交事务）：
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/707a124e0d76-202404261856974.png)
...
![](/imported/markdown/2025-08-09-markdown-21569f3a-mysql事务原理-从acid到隔离级别的全解析/images/b067073043fc-202404261856976.png)
