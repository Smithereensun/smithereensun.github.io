{
  ...
  "has_date": true,
  "description": "介绍 在ZooKeeper集群服中务中有三个角色： Leader领导者 ： 处理事务请求(即读和写服务) 集群内部各服务器的调度者 Follower跟随者： 处理客户端非事务请求(即读服务)，转发事务请求给Leader服务器 参与Leader选举投票 Observer观察者： 处理客户端非事务请求(",
  "tags": [
    "微服务",
    "ZooKeeper"
  ],
  "source": "local-markdown-library",
  "source_path": "microservices/service-registration-and-discovery/zookeeper-colony - ZooKeeper - 集群机制.md",
  "date": "2025-05-17"
...
## [ZK集群 Leader 选举机制](#zk集群-leader-选举机制)
...
1. Synchronization（同步阶段） :同步阶段主要是利用 leader 前一阶段获得的最新提议历史，同步集群中所有的副本。同步完成之后准 leader 才会成为真正的 leader。

1. Broadcast（广播阶段） :到了这个阶段，ZooKeeper 集群才能正式对外提供事务服务，并且 leader 可以进行消息广播。同时如果有新的节点加入，还需要对新节点进行同步。

### [服务器初始化时Leader选举](#服务器初始化时leader选举)
...
### [服务器运行期间的Leader选举](#服务器运行期间的leader选举)
