{
  ...
  "has_date": false,
  "description": "日常开发中，我们经常遇到这种业务场景，如：外卖订单超 30 分钟未支付，则自动取订单；用户注册成功 15 分钟后，发短信息通知用户等等。这就是延时任务处理场景。 在电商，支付等系统中，一设都是先创建订单（支付单）,再给用户一定的时间进行支付，如果没有按时支付的话，就需要把之前的订单（支付单）取消掉。",
  "tags": [
    "面试",
    "系统设计"
  ],
  "source": "local-markdown-library",
  "source_path": "interview/system-design/system-design/how-to-achieve-automatic-order-closure - 电商平台中订单未支付过期如何实现自动关单？.md"
...
## [JDK 延迟队列 DelayQueue](#jdk-延迟队列-delayqueue)

DelayQueue是JDK提供的一个无界队列，DelayQueue队列中的元素需要实现Delayed，它只提供了一个方法，就是获取过期时间。
![](/imported/markdown/undated-markdown-d756a9d6-电商平台中订单未支付过期如何实现自动关单/images/240a11c100c1-202410271945718.png)
...
## [Redisson分布式延迟队列](#redisson分布式延迟队列)
...
![](/imported/markdown/undated-markdown-d756a9d6-电商平台中订单未支付过期如何实现自动关单/images/5f5e9eb28f50-202410272003128.png)
...
### [为什么不建议用MQ实现订单到期关闭](#为什么不建议用mq实现订单到期关闭)
...
1. 如果订单之前没有更新过状态了，可以更新并完成相关业务逻辑，否则本次不能更新，也不能完成业务逻辑。

1. 释放当前订单的锁。

![](/imported/markdown/undated-markdown-d756a9d6-电商平台中订单未支付过期如何实现自动关单/images/4a3d6d6f1f98-202411161142482.webp)![](/imported/markdown/undated-markdown-d756a9d6-电商平台中订单未支付过期如何实现自动关单/images/8ee93d977351-202411161142586.webp)
### [兜底意识 + 配置监控](#兜底意识-配置监控)
...
![](/imported/markdown/undated-markdown-d756a9d6-电商平台中订单未支付过期如何实现自动关单/images/185182437a43-202411161142574.webp)性能监控
