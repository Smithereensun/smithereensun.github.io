{
  ...
  "has_date": true,
  "description": "Spring框架系列文章建议阅读顺序：全网最详细的Spring入门教程 -》Spring框架IoC核心详解 -》 Spring AOP基础、快速入门 -》 MVC快速入门；而后再阅读其它文章详细内容 为什么用Spring 什么是Spring Spring 是一款开源的轻量级 Java 开发框架，旨在",
  "tags": [
    "框架",
    "Spring"
  ],
  "source": "local-markdown-library",
  "source_path": "framework/spring/spring-summary - 全网最详细的Spring入门教程.md",
  "date": "2025-05-17"

}

Spring框架系列文章建议阅读顺序：全网最详细的Spring入门教程 -》Spring框架IoC核心详解 -》 Spring AOP基础、快速入门 -》 MVC快速入门；而后再阅读其它文章详细内容
...
## [Spring的组件](#spring的组件)
![Spring5.x主要模块](/imported/markdown/2025-05-17-markdown-ff2318df-全网最详细的spring入门教程/images/7a4afb889f03-202407201016227.png)Spring5.x主要模块
Spring5.x 版本中 Web 模块的 Portlet 组件已经被废弃掉，同时增加了用于异步响应式处理的 WebFlux 组件。
![Spring 各个模块的依赖关系](/imported/markdown/2025-05-17-markdown-ff2318df-全网最详细的spring入门教程/images/33f909aff66d-202407201016817.png)Spring 各个模块的依赖关系
从最下层往上介绍
...
源码对应模块如下：
![](/imported/markdown/2025-05-17-markdown-ff2318df-全网最详细的spring入门教程/images/5344dac185c8-202407201048121.png)
...
对应源码模块如下：
![](/imported/markdown/2025-05-17-markdown-ff2318df-全网最详细的spring入门教程/images/4055674a9667-202407201046700.png)
### [AOP、Aspects、Instrumentation和Messaging](#aop、aspects、instrumentation和messaging)
...
对应源码模块如下：
![](/imported/markdown/2025-05-17-markdown-ff2318df-全网最详细的spring入门教程/images/7febd571678b-202407201051187.png)
...
对应源码模块：
![](/imported/markdown/2025-05-17-markdown-ff2318df-全网最详细的spring入门教程/images/2b53fa711bfe-202407201055987.png)
...
对应源码模块如下：
![](/imported/markdown/2025-05-17-markdown-ff2318df-全网最详细的spring入门教程/images/87e97a95267e-202407201102080.png)
## [Spring、SpringMVC、SpringBoot之间的关系](#spring、springmvc、springboot之间的关系)
...
案例源码点击[这里](https://github.com//Spring-Demo/tree/master/01-spring-framework-helloworld-xml)
...
运行结果：
![](/imported/markdown/2025-05-17-markdown-ff2318df-全网最详细的spring入门教程/images/4679747ef1c5-202407201420026.png)
### [如何体现的Spring优势](#如何体现的spring优势)
...
1. Spring 框架托管创建的Bean放在哪里呢？ 这便是**IoC Container**;

1. Spring 框架为了更好让用户配置Bean，必然会引入**不同方式来配置Bean？ 这便是xml配置，Java配置，注解配置**等支持

1. Spring 框架既然接管了Bean的生成，必然需要**管理整个Bean的生命周期**等；

1. 应用程序代码从Ioc Container中获取依赖的Bean，注入到应用程序中，这个过程叫 **依赖注入(Dependency Injection，DI)** ； 所以说控制反转是通过依赖注入实现的，其实它们是同一个概念的不同角度描述。通俗来说就是**IoC是设计思想，DI是实现方式**

1. 在依赖注入时，有哪些方式呢？这就是构造器方式，@Autowired, @Resource, @Qualifier... 同时Bean之间存在依赖（可能存在先后顺序问题，以及**循环依赖问题**等）
...
1. 为什么@Aspect注解使用的是aspectj的jar包呢？这就引出了**Aspect4J和Spring AOP的历史渊源**，只有理解了Aspect4J和Spring的渊源才能理解有些注解上的兼容设计

1. 如何支持**更多拦截方式**来实现解耦， 以满足更多场景需求呢？ 这就是@Around, @Pointcut... 等的设计

1. 那么Spring框架又是如何实现AOP的呢？ 这就引入**代理技术，分静态代理和动态代理**，动态代理又包含JDK代理和CGLIB代理

## [Spring框架逐步简化开发](#spring框架逐步简化开发)

### [Java 配置方式改造](#java-配置方式改造)

案例源码点击[这里](https://github.com//Spring-Demo/tree/master/02-spring-framework-helloworld-config)
...
1.
当应用启动时，Spring框架会使用Java的反射API来检查所有带有`@Configuration` 注解的类（这里不懂的可以看下注解实现的原理）。Spring框架内置了一个注解处理器`ConfigurationClassPostProcessor`，它是`BeanFactoryPostProcessor` 的一个实现。

1.
接着`ConfigurationClassPostProcessor` 会在容器初始化时被调用，它会查找所有带有`@Configuration` 注解的类，并解析这些类中定义的`@Bean` 方法。
...
1.
**注册BeanDefinition**： 一旦所有的`BeanDefinition` 被创建，它们会被注册到Spring容器的`BeanFactory` 中。这样，Spring容器就可以在需要时创建和注入这些Bean。
...
案例源码点击[这里](https://github.com//Spring-Demo/tree/master/03-spring-framework-helloworld-anno)
...
- 在App中扫描com..springhelloworldanno包

这里要提一嘴的是，现在大多数的Spring项目，基本都是这种方式。主要步骤就是：
 1、对类添加@Component相关的注解，比如@Controller，@Service，@Repository
 2、设置ComponentScan的basePackage, 比如在xml文件里设置`&lt;context:component-scan base-package='com..springframework'&gt;`, 或者在配置类中设置`@ComponentScan("com..springframework")`注解，或者 直接在APP类中 `new AnnotationConfigApplicationContext("com..springframework")`指定扫描的basePackage.
