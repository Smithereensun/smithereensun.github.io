{
  ...
  "title": "Redis集群机制深度剖析",
  "has_date": true,
  "description": "概述 Redis单实例的架构，从最开始的一主N从，到读写分离，再到Sentinel哨兵机制，单实例的Redis缓存足以应对大多数的使用场景，也能实现主从故障迁移。 但是，在某些场景下，单实例存Redis缓存会存在的几个问题： 写并发：Redis单实例读写分离可以解决读操作的负载均衡，但对于写操作，仍",
  "tags": [
    "数据库",
    "Redis"
  ],
  "source": "local-markdown-library",
  "source_path": "database/redis/04-ha3-colony - Redis集群机制深度剖析.md",
  "date": "2025-05-17"
...
![](/imported/markdown/2025-05-17-markdown-80dc5e6c-redis集群机制深度剖析/images/61faada66efb-202411301318769.png)
...
1. 通过水平增加多个分片的方式，可以实现整体集群的容量的扩展。
...
### [Redis Sharding（数据分片）](#redis-sharding-数据分片)
...
![](/imported/markdown/2025-05-17-markdown-80dc5e6c-redis集群机制深度剖析/images/b9c48607f9fc-202411301318343.png)
...
![](/imported/markdown/2025-05-17-markdown-80dc5e6c-redis集群机制深度剖析/images/e0e8c74209f2-202411301318133.png)
...
![](/imported/markdown/2025-05-17-markdown-80dc5e6c-redis集群机制深度剖析/images/3e365e59d75c-202411301317804.png)
...
### [MOVED请求](#moved请求)
![](/imported/markdown/2025-05-17-markdown-80dc5e6c-redis集群机制深度剖析/images/04fa01a9e527-202406152047331.png)
...
![](/imported/markdown/2025-05-17-markdown-80dc5e6c-redis集群机制深度剖析/images/a1a331d83b5c-202406152047441.png)
...
### [频繁重定向造成的网络开销的处理：smart客户端](#频繁重定向造成的网络开销的处理-smart客户端)

#### [什么是 smart客户端：](#什么是-smart客户端)
...
#### [JedisCluster的工作原理：](#jediscluster的工作原理)
...
#### [hashslot迁移和ask重定向：](#hashslot迁移和ask重定向)
...
## [Redis集群中节点的通信机制：goosip协议](#redis集群中节点的通信机制-goosip协议)
...
1. 使用cluster meet命令将新节点加入到集群

1. 迁移槽和数据：添加新节点后，需要将一些槽和数据从旧节点迁移到新节点

![](/imported/markdown/2025-05-17-markdown-80dc5e6c-redis集群机制深度剖析/images/23fa9eafaca0-202406152125451.png)
...
![](/imported/markdown/2025-05-17-markdown-80dc5e6c-redis集群机制深度剖析/images/b1324cd40800-202406152125460.png)
...
1. 之后对源节点发起送命令，让源节点准备迁出对应的槽数据。使用命令 cluster setslot {slot} importing {sourceNodeId}。

1. 此时源节点准备迁移数据了，在迁移之前把要迁移的数据获取出来。通过命令 cluster getkeysinslot {slot} {count}。Count 表示迁移的 Slot 的个数。

1. 然后在源节点上执行，migrate {targetIP} {targetPort} “” 0 {timeout} keys {keys} 命令，把获取的键通过流水线批量迁移到目标节点。

1. 重复 3 和 4 两步不断将数据迁移到目标节点。

1. 完成数据迁移到目标节点以后，通过 cluster setslot {slot} node {targetNodeId} 命令通知对应的槽被分配到目标节点，并且广播这个信息给全网的其他主节点，更新自身的槽节点对应表。

### [收缩](#收缩)

1. 迁移槽。

1. 忘记节点。通过命令 cluster forget {downNodeId} 通知其他的节点

![](/imported/markdown/2025-05-17-markdown-80dc5e6c-redis集群机制深度剖析/images/6875aa1e4920-202406152125478.png)
...
投票选举：
  ...
  1.
节点排序：对通过过滤条件的所有从节点进行排序，按照priority、offset、run id排序，排序越靠前的节点，越优先进行选举。 (在这步排序领先的从节点通常会获得更多的票，因为它触发选举的时间更早一些，获得票的机会更大)
