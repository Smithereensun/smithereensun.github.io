{
  ...
  "has_date": true,
  "description": "概述 在应用Sychronized关键字时需要把握如下注意点： 一把锁只能同时被一个线程获取，没有获得锁的线程只能等待； 每个实例都对应有自己的一把锁(this),不同实例之间互不影响；例外：锁对象是*.class以及synchronized修饰的是static方法的时候，所有对象公用同一把锁 sy",
  "tags": [
    "Java",
    "并发",
    "JVM"
  ...
  "source_path": "java/concurrent/02-keyword1-synchronized - synchronized底层机制剖析：从字节码到JVM的运作原理.md",
  "date": "2025-05-17"
...
![](/imported/markdown/2025-05-17-markdown-077d81df-synchronized底层机制剖析-从字节码到jvm的运作原理/images/403f87441022-202404251023096.jpg)![](/imported/markdown/2025-05-17-markdown-077d81df-synchronized底层机制剖析-从字节码到jvm的运作原理/images/acc0bfe09ae1-202404251023120.gif)
...
1. 如果其他线程已经占用了monitor，则该线程进入阻塞状态，直到monitor的进入数为0，再重新尝试获取monitor的所有权
...
编译之后反编译截图：
![](/imported/markdown/2025-05-17-markdown-077d81df-synchronized底层机制剖析-从字节码到jvm的运作原理/images/4924cc63aa9b-202404251023111.gif)
...
![](/imported/markdown/2025-05-17-markdown-077d81df-synchronized底层机制剖析-从字节码到jvm的运作原理/images/5eac5ab93f0d-202404251023090.jpg)
...
![](/imported/markdown/2025-05-17-markdown-077d81df-synchronized底层机制剖析-从字节码到jvm的运作原理/images/fc9116783713-202404251023099.jpg)
### [Java对象头](#java对象头)
...
![64位Mark Word的结构信息](/imported/markdown/2025-05-17-markdown-077d81df-synchronized底层机制剖析-从字节码到jvm的运作原理/images/1d58396193ac-202409211310009.png)64位Mark Word的结构信息
关于对象头的详细内容可以看Java对象头这篇文章

## [synchronized锁的优化](#synchronized锁的优化)
...
![](/imported/markdown/2025-05-17-markdown-077d81df-synchronized底层机制剖析-从字节码到jvm的运作原理/images/59893380a8ab-202404251023238.gif)

1. 刚开始Monitor中Owner为null

1. 当Thread-2执行Synchronized(obj)就会将Monitor的所有者owner置为Thread-2。Monitor中只能有一个Owner。

1. 在Thread-2上锁的过程中，如果Thread-3，Thread-4，Thread-5也来执行Synchronized(obj)，就会进入EntryList Blocked。

1. Thread-2执行完同步代码块的内容，然后唤醒EntryList中等待的线程来竞争锁，竞争时是非公平的

1. 图中WaitSet中的Thread-0，Thread-1是之前获得过锁，但条件不满足而进入waiting状态的线程
...
![](/imported/markdown/2025-05-17-markdown-077d81df-synchronized底层机制剖析-从字节码到jvm的运作原理/images/5a28c06e4590-202404251023264.gif)
### [锁粗化](#锁粗化)
...
上述代码使用javap 编译结果
![](/imported/markdown/2025-05-17-markdown-077d81df-synchronized底层机制剖析-从字节码到jvm的运作原理/images/ac6f9f29f396-202404251023288.jpg)
...
1. 若为可偏向状态，则测试Mark Word中的线程ID是否与当前线程相同，若相同，则直接执行同步代码，否则进入下一步。

1. 当前线程通过CAS操作竞争锁，若竞争成功，则将Mark Word中线程ID设置为当前线程ID，然后执行同步代码，若竞争失败，进入下一步。

1. 当前线程通过CAS竞争锁失败的情况下，说明有竞争。当到达全局安全点时之前获得偏向锁的线程被挂起，偏向锁升级为轻量级锁，然后被阻塞在安全点的线程继续往下执行同步代码。
...
1. 复制对象头中的Mark Word到锁记录中。

1. 复制成功后，虚拟机将用CAS操作将对象的Mark Word更新为执行Lock Record的指针，并将Lock Record里的owner指针指向对象的Mark Word。如果更新成功，则执行4，否则执行5。；

1. 如果更新成功，则这个线程拥有了这个锁，并将锁标志设为00，表示处于轻量级锁状态

1. 如果更新失败，虚拟机会检查对象的Mark Word是否指向当前线程的栈帧，如果是则说明当前线程已经拥有这个锁，可进入执行同步代码。否则说明多个线程竞争，轻量级锁就会膨胀为重量级锁，Mark Word中存储重量级锁（互斥锁）的指针，后面等待锁的线程也要进入阻塞状态。
...
### [锁的优缺点](#锁的优缺点)
锁优点缺点使用场景偏向锁加锁和解锁不需要CAS操作，没有额外的性能消耗，和执行非同步方法相比仅存在纳秒级的差距如果线程间存在锁竞争，会带来额外的锁撤销的消耗适用于只有一个线程访问同步块的场景轻量级锁竞争的线程不会阻塞，提高了响应速度如线程始终得不到锁竞争的线程，使用自旋会消耗CPU性能追求响应时间，同步块执行速度非常快重量级锁线程竞争不适用自旋，不会消耗CPU线程阻塞，响应时间缓慢，在多线程下，频繁的获取释放锁，会带来巨大的性能消耗追求吞吐量，同步块执行速度较长
### [锁升级过程](#锁升级过程)

1. 无锁：无竞争

1. 偏向锁：长时间只有一个线程访问，就会获得偏向锁

1. 轻量级锁：当前锁是偏向锁并且被另外一个线程访问时，偏向锁就会升级为轻量级锁

1. 重量级锁：若当前只有一个等待线程，则该线程通过自旋进行等待。但是当自旋超过一定的次数，或者一个线程在持有锁，一个在自旋，又有第三个来访时，轻量级锁升级为重量级锁。

## [Synchronized与ReentrantLock](#synchronized与reentrantlock)
![](/imported/markdown/2025-05-17-markdown-077d81df-synchronized底层机制剖析-从字节码到jvm的运作原理/images/30d3492ebb7b-202404251023310.jpg)
...
### [Synchronized使用注意点](#synchronized使用注意点)
...
1. 在能选择的情况下，既不要用Lock也不要用synchronized关键字，用java.util.concurrent包中的各种各样的类，如果不用该包下的类，在满足业务的情况下，可以使用synchronized关键，因为代码量少，避免出错
