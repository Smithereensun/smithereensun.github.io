{
  ...
  "has_date": true,
  "description": "Maven 项⽬⽣命周期 Maven从项⽬的三个不同的⻆度，定义了三套⽣命周期，三套⽣命周期是相互独⽴的，它们之间不会相互影响。 清理⽣命周期(Clean Lifecycle)：该⽣命周期负责清理项⽬中的多余信息，保持项⽬资源和代码的整洁性。⼀般拿来清空directory(即⼀般的target)⽬录",
  "tags": [
    "工具",
    "Maven"
  ],
  "source": "local-markdown-library",
  "source_path": "tools/maven-detail - Maven入门，读完这篇就够了.md",
  "date": "2025-05-17"

}

## [Maven 项⽬⽣命周期](#maven-项目生命周期)

Maven从项⽬的三个不同的⻆度，定义了三套⽣命周期，三套⽣命周期是相互独⽴的，它们之间不会相互影响。
![](/imported/markdown/2025-05-17-markdown-aa4b530d-maven入门-读完这篇就够了/images/ba02f1089b83-202407192157695.png)
...
##### [scope](#scope)
依赖范围编译有效测试有效运行时有效打包有效例子Complie√√√√spring-coretest×√××Junitprovided√√××servlet-api,lombokruntime×√√√JDBC驱动system√√××本地maven仓库之外的类库importN/AN/AN/AN/ABOM文件
##### [optional](#optional)
...
![](/imported/markdown/2025-05-17-markdown-aa4b530d-maven入门-读完这篇就够了/images/f3cf541ae452-202404270842208.png)
...
#### [冲突产生的根本原因](#冲突产生的根本原因)
![](/imported/markdown/2025-05-17-markdown-aa4b530d-maven入门-读完这篇就够了/images/cc418dbe87c7-202404270842062.png)
由于传递依赖的原因，a会通过b引入c的依赖，也会通过d引入c的依赖，因此出现了冲突
依赖关系实例直接依赖a和b的依赖关系间接依赖a和c的依赖关系
#### [依赖冲突的解决方案](#依赖冲突的解决方案)
...
1. 发现了冲突的包之后，剩下的就是选择⼀个合适版本的包留下，如果是传递依赖的包正确，那么把显示依赖的包exclude掉。如果是某⼀个传递依赖的包有问题，那需要⼿动把这个传递依赖execlude掉

## [如何处理无法拉取的jar包](#如何处理无法拉取的jar包)
...
![](/imported/markdown/2025-05-17-markdown-aa4b530d-maven入门-读完这篇就够了/images/07ae14465d0b-202404270843946.png)
### [配置文件问题](#配置文件问题)
...
![](/imported/markdown/2025-05-17-markdown-aa4b530d-maven入门-读完这篇就够了/images/e7011e2faa6d-202404270843327.png)
找到所需的jar包，进行下载
![](/imported/markdown/2025-05-17-markdown-aa4b530d-maven入门-读完这篇就够了/images/1799145c1494-202404270843124.png)
并放到本地的maven仓库中
